...
function sendAccessCardInfoExListReq(isList){
	...
	sms_getAccessCardInfoExList.setParameters("userType", app.lookup("AMCIS_cmbUserType").value);
	...
	sms_getAccessCardInfoExList.setParameters("cardTypeEx", CivilServiceCardNormal);
	...
	if (resultCode == COMERROR_NONE) {
		...
		if (accessCardList.getRowCount() == 0) {
			...
			for (var i = 0; i < count; i++) {
				...
				accessCard.setValue("ResitAt", validateDate(accessCard.getValue("ResitAt")));
			}
...
function exportExcel() {
	...
	stringified = stringified.replace(/"CardNumber"/gi, '"공무원증번호"');
	...
	var today = dateLib.getToday();
	var filename = "공무원증_등록현황_" + today + ".xlsx";
	var ws_name = "공무원증_등록현황";
