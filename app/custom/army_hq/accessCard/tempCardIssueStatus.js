...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	var cmbGroup = app.lookup("AMAIS_cmbGroup");
...
function sendAccessCardListReq(isList) {
	...
	sms_getAccessCardInfoList.setParameters("cardType", AccessCardTypeTempory);
...
function onSms_getAccessCardUserInfoListSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	...
	if(resultCode == COMERROR_NONE) {
		...
		for(var i=0; i<count; i++){
			...
			accessCard.setValue("RetriveAt", validateDate(accessCard.getValue("RetriveAt")));
		}
...
function exportExcel() {
	...
	stringified = stringified.replace(/"RetriveAt"/gi, '"회수일"');
	...
	var today = dateLib.getToday();
	var filename = "임시출입증_발급현황_" + today + ".xlsx";
	var ws_name = "임시출입증_발급현황";
