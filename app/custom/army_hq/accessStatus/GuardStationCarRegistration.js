/************************************************
 * GuardStationCarRegistration.js
 * Created at 2021. 2. 2. 오후 2:19:36.
 *
 * @author blue1
 ************************************************/
...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	srcTitle = new Array();
	var grdVisitInfo = app.lookup("ASR_grdCarAccessRegistration");
	...
	// 위병소 차량출입 일괄등록 excel 양식
	var link = app.lookup("AMGCR_sniDownloadLink");
	link.value=	"<a href=\"/setup/custom_armyhq/위병소차량출입일괄등록양식.xlsx\" target=\"_blank\">양식 다운로드</a>";
}

/* 버튼 클릭 이벤트 */
function onDeleteLineButtonClick(/* cpr.events.CMouseEvent */ e){
	var dataSet = app.lookup("dsCarAccessInfo");
...
function onRegistButtonClick(/* cpr.events.CMouseEvent */ e){
	...
	submission.action = "/v1/armyhq/manualAccessStatus/" + CarAccessRegistration;
	submission.send();
}
 ...
 */
function onAMGCR_fiUserFileValueChange(/* cpr.events.CValueChangeEvent */ e){
	/** 
    ...
    for (i = 0; i != files.length; ++i) {
        ...
        reader.onload = function(e) {
			...
			app.getRootAppInstance().openDialog(appld, {width : 480, height : 600}, function(dialog){
				...
				var dsUserList = app.lookup("dsCarAccessInfo");						
				...
				var accessAreaItems = app.lookup("AMGCR_cmbAccessArea").getItems();
				var accessTypeItems = app.lookup("AMGCR_cmbAccessType").getItems();
				var carTypeItems = app.lookup("AMGCR_cmbCarType").getItems();
				...
				for(var i=0;i<userList.length;i++){
					...
					carTypeItems.forEach(function(item) {
						if (item.label == userList[i][dataManager.getString("Str_ARMY_CarType")]){
							row.setValue("CarType", item.value);
						} 
					});					
