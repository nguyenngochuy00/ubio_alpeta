...
function onBodyLoad( /* cpr.events.CEvent */ e) {
	...
	updateLogoData();	
	// onMenuChange(null,"app/custom/army_hq/layout/visitApplicationManagement");
	var sms_getLoginInfo = app.lookup("sms_getLoginInfo");
	sms_getLoginInfo.send();
	
	var embView = app.lookup("AHNMT_eaMainView");
	var srcPath = "app/custom/army_hq/mainPageAMHQ";
...
function updateLogoData(){
	...
	if( siteName && siteName.length>0){
		...
		app.lookup("AHNMT_opbSiteTitle").value = "과학화 출입통제 체계";
	}
	...
	if( siteLogo && siteLogo.length>0){
		...
		app.lookup("AHNMT_imbSiteLogo").style.css({"background-image" : "url('../theme/custom/armyhq/sign_img_logo_01.png')", "mix-blend-mode": "multiply"})		
	}
}
...
exports.openSubMenu = function( menuID,subMenuID){
	...
	switch(subMenuID){
		// 방문신청관리
		case "10201": src = "app/custom/army_hq/layout/visitApplicationManagement"; break;
		
		// 출입신청관리	
		case "20201": src = "app/custom/army_hq/layout/accessApplicationManagement"; break;
		case "20601": src = "app/custom/army_hq/layout/accessApplicationManagement"; break;		
		case "20702": src = "app/custom/army_hq/layout/accessApplicationManagement"; break;
		
		//장소/장비관리
		case "40201": src = "app/custom/army_hq/layout/placeEquipmentManagement"; break;
		case "40203": src = "app/custom/army_hq/layout/placeEquipmentManagement"; break;
		...
		default:
			...
			if (topMenu.getChildrenCount() == 0) {
				console.log("child 0!!")
				menuID = 0
				src = "app/custom/army_hq/blankPage";
			} else {
				switch(topMenu.getFirstChild().value) {
					case "외래인방문신청관리": 
						menuID = 10;
						src = "app/custom/army_hq/layout/visitApplicationManagement";
						break;
					case "출입신청관리":
						menuID = 20; 
						src = "app/custom/army_hq/layout/accessApplicationManagement";
						break;
					case "출입현황": 
						menuID = 30;
						src = "app/custom/army_hq/layout/accessStatus";
						break;
					case "장소/장비관리": 
						menuID = 40;
						src = "app/custom/army_hq/layout/placeEquipmentManagement";
						break;
					case "모니터링": 
						menuID = 50;
						src = "app/custom/army_hq/layout/monitoring";
						break;
					case "시스템관리": 
						menuID = 60;
						src = "app/custom/army_hq/layout/systemManagement";
						break;												
				}
			}
	}
