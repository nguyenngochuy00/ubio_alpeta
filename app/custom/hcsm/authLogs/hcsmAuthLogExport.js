...
function exportExcel() {
	...
	for (var i = 0; i < total; i++) {
		...
		//in/out
		var inOut = authLogInfo.getValue("Latitude");
		var inOutName;
		switch (inOut) {
			case "1":
				inOutName = "OUT";
		...
		authLogInfo.setValue("Latitude", inOutName);
		
		//boolid
		var bloodID = authLogInfo.getValue("BloodID");
		var bloodTypeInfo = dataManager.getBloodType(parseInt(bloodID));
		if (bloodTypeInfo) {
			authLogInfo.setValue("BloodID", bloodTypeInfo.getValue("BloodName"));
		} else {
			authLogInfo.setValue("BloodID", "");
		}
	...
	stringified = stringified.replace(/"IqamaNo"/gi, '"' + dataManager.getString("Str_IqamaNo") + '"');
	stringified = stringified.replace(/"Mobile"/gi, '"' + dataManager.getString("Str_MobileNum") + '"');
	stringified = stringified.replace(/"Remarks"/gi, '"' + dataManager.getString("Str_Remarks") + '"');
	stringified = stringified.replace(/"BloodID"/gi, '"' + dataManager.getString("Str_BloodType") + '"');
	stringified = stringified.replace(/"Latitude"/gi, '"' + dataManager.getString("Str_InOutput") + '"');
 ...
 */
function onALEMP_udcAuthlogListPagechange2( /* cpr.events.CSelectionEvent */ e) {
	/** 
	 * @type udc.grid.authLogListHCSM
	 */
