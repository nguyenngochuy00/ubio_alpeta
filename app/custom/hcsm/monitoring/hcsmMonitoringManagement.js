...
function onBodyLoad( /* cpr.events.CEvent */ e) {
	...
	if (terminalList) {
		...
		for (var i = 0; i < dsTerminalList.getRowCount(); i++) {
			...
			if (terminalInfo.getValue("Status") == 0) {
				terminalInfo.setValue("Event", 1);
			}
...
function initComboAuthLog() {
	...
	var cmbBloodType = app.lookup("MRMAN_cmbBloodType");
	
	cmbBloodType.setItemSet(dataManager.getBloodTypeList(), {
		label: "BloodName",
		value: "BloodID",
	});
	var cmbInOut = app.lookup("MRMAN_cmbInOut");
	cmbInOut.addItem(new cpr.controls.Item("OUT", 1));
	cmbInOut.addItem(new cpr.controls.Item("IN", 2));
...
function onStatusUpdate(category, value, terminal) {
	...
	switch (category) {
		case 0: // TerminalStatusConnect 1<<0
			if (value == 1) {
				terminal.setValue("TerminalStatus1", TerminalStatusConnect);
			} else {
				terminal.setValue("TerminalStatus1", "");
		...
		case 1: // TerminalStatusLock         = 1 << 1 // 잠김 - 0:해제, 1:잠김
			if (value == 1) {
				terminal.setValue("TerminalStatus3", TerminalStatusLock);
			}
		...
		case 2: //TerminalStatusLockForce    = 1 << 2 // 폐쇄 - 0:정상, 1:폐쇄
			if (value == 1) {
				terminal.setValue("TerminalStatus3", TerminalStatusLockForce);
			}
			//else{	terminal.setValue("TerminalStatus3", "");}
			break;
		...
		case 3:
			if (value == 1) {
				terminal.setValue("TerminalStatus3", TerminalStatusCover);
			}
			//else{	terminal.setValue("TerminalStatus3", "");}
			break;
		...
		case 5:
			if (value == 1) {
				terminal.setValue("TerminalStatus1", TerminalStatusIDConflict);
			}
			break;
			
		case 6:
			if (value == 1) {
				terminal.setValue("TerminalStatus1", TerminalStatusInvalidType);
			}
			break;
			
		case 7:
			if (value == 1) {
				terminal.setValue("TerminalStatus2", TerminalStatusDoorOpen);
			} else {
				terminal.setValue("TerminalStatus2", TerminalStatusDoorClose);
			}
			break;
			
		case 8:
			if (value == 1) {
				terminal.setValue("TerminalStatus2", TerminalStatusDoorEmergency);
			}
		...
		case 9:
			if (value == 1) {
				terminal.setValue("TerminalStatus2", TerminalStatusDoorOpenWarn);
			}
			//else{	terminal.setValue("TerminalStatus2", "");}
			break;
		...
		case 10:
			if (value == 1) {
				terminal.setValue("TerminalStatus2", TerminalStatusDoorLookState);
			}
			//else{	terminal.setValue("TerminalStatus2", "");}
			break;
		...
		case 11:
			if (value == 1) {
				terminal.setValue("TerminalStatus2", TerminalStatusDoorLockWorking);
			}
			//else{	terminal.setValue("TerminalStatus2", "");}
			break;
		case 12:
			if (value == 1) {
				terminal.setValue("TerminalStatus4", TerminalStatusWarnFire);
			}
		...
		case 13:
			if (value == 1) {
				terminal.setValue("TerminalStatus4", TerminalStatusWarnPanic);
			}
			//else{	terminal.setValue("TerminalStatus4", "");}
			break;
		...
		case 14:
			if (value == 1) {
				terminal.setValue("TerminalStatus4", TerminalStatusWarnCricis);
			}
			//else{	terminal.setValue("TerminalStatus4", "");}
			break;
		...
		case 15:
			if (value == 1) {
				terminal.setValue("TerminalStatus1", TerminalStatusSyncUserIDLen);
			}
			break;
			
		case 16:
			if (value == 1) {
				terminal.setValue("TerminalStatus1", TerminalStatusSyncMacAddres);
			}
			break;
			
		case 17:
			if (value == 1) {
				terminal.setValue("TerminalStatus1", TerminalStatusSyncUserCount);
			}
			break;
			
		case 18:
			if (value == 1) {
				terminal.setValue("TerminalStatus1", TerminalStatusSyncFWVersion);
			}
			break;
		case 23:
			if (value == 1) {
				terminal.setValue("TerminalStatus2", TerminalStatusDoorNotMonitoring);
			}
			//else{	terminal.setValue("TerminalStatus2", "");}
			break;
		case 24:
...
exports.addAuthLog = function(authLog) {
	...
	if (authLog.ReserveData.length > 4) {
		...
		if (authLog.ReserveType == 1) { // 열화상
			...
			alcolValue = alcolValue.toFixed(3);
			insertRow.setValue("Detail", temp + data[1] + "." + alcolValue);
		}
	}
...
function setTerminalEventVal(terminalID, eventVal) {
	...
	if (terminal) {
		...
		switch (eventVal) {
			//Status1
			case EventLogTerminalDisconnected:
				terminal.setValue("TerminalStatus1", "");
				break;
			case 65538:
				if (status == "1") {
...
function processSyncCommand() {
	...
	if (terminalRow) {
		...
		app.getRootAppInstance().openDialog(appld, {
			...
			dialog.initValue = {
				"ID": terminalRow.getValue("ID"),
				"TimezoneVersion": terminalRow.getValue("TimezoneVersion")
			};
			dialog.modal = true;
		}).then(function(input) {
			;
			...
			if (e.rowIndex == i && e.cellIndex == 4) {
				switch (row.getValue("TerminalStatus1")) {
					case TerminalStatusConnect:
						e.control.tooltip = dataManager.getString("Str_Connect");
						break; //연결
					case TerminalStatusRegist:
						e.control.tooltip = dataManager.getString("Str_Regist");
						break; // 등록
					case TerminalStatusUnRegist:
						e.control.tooltip = dataManager.getString("Str_UnRegist");
						break; // 미등록
					case TerminalStatusIDConflict:
						e.control.tooltip = dataManager.getString("Str_IDConflict");
						break; //아이디 출동
					case TerminalStatusInvalidType:
						e.control.tooltip = dataManager.getString("Str_TerminalInvalidType");
						break; //등록된 단말기 타입이 다름
					case TerminalStatusSyncUserIDLen:
						e.control.tooltip = dataManager.getString("Str_UserIDLen");
						break; //사용자 아이디 길이
					case TerminalStatusSyncMacAddres:
						e.control.tooltip = dataManager.getString("Str_MacAddres");
						break; //맥 주소
					case TerminalStatusSyncUserCount:
						e.control.tooltip = dataManager.getString("Str_UserCount");
						break; //사용자 수
					case TerminalStatusSyncFWVersion:
						e.control.tooltip = dataManager.getString("Str_FWVersion");
						break; //펌웨어 버전
					default:
						e.control.tooltip = "";
						break;
				...
				switch (row.getValue("TerminalStatus2")) {
					case TerminalStatusDoorOpen:
						e.control.tooltip = dataManager.getString("Str_DoorOpen");
						break; //(도어 - 0:닫힘, 1:열림) : 출입문 열림
					case TerminalStatusDoorClose:
						e.control.tooltip = dataManager.getString("Str_DoorClose");
						break; // (도어 - 0:닫힘, 1:열림) : 출입문 닫힘
					case TerminalStatusDoorEmergency:
						e.control.tooltip = dataManager.getString("Str_DoorEmergency");
						break; //(침입 - 0:정상, 1:침입) : 강제 침입 
					case TerminalStatusDoorOpenWarn:
						e.control.tooltip = dataManager.getString("Str_DoorOpenWarn");
						break; // (방치 - 0:정상, 1:방치) : 문열림 경고
					case TerminalStatusDoorLookState:
						e.control.tooltip = dataManager.getString("Str_DoorLookState");
						break; // (상태 - 0:잠김, 1:열림) : 출입문 열림
					case TerminalStatusDoorLockWorking:
						e.control.tooltip = dataManager.getString("Str_DoorLockWorking");
						break; // (동작 - 0:정상, 1:고장) : 문열림 경고   
					case TerminalStatusDoorNotMonitoring:
						e.control.tooltip = dataManager.getString("Str_DoorNotMonitor");
						break; // 미감시 – 0:정상 , 1: 미감시 : 문상태 미감시
					case TerminalStatusDoorOpenState:
						e.control.tooltip = dataManager.getString("Str_DoorCommandOpen");
				...
				switch (row.getValue("TerminalStatus3")) {
					case TerminalStatusLock:
						e.control.tooltip = dataManager.getString("Str_Lock");
						break; // (잠김 - 0:해제, 1:잠김) : 문열림 경고    
					case TerminalStatusLockForce:
						e.control.tooltip = dataManager.getString("Str_LockForce");
						break; // (폐쇄 - 0:정상, 1:폐쇄) : 비상 
					case TerminalStatusCover:
						e.control.tooltip = dataManager.getString("Str_CoverSeparate");
				...
				switch (row.getValue("TerminalStatus4")) {
					case TerminalStatusWarnFire:
						e.control.tooltip = dataManager.getString("Str_WarnFire");
						break; // (화재 - 0:정상, 1:화재) : 화재 
					case TerminalStatusWarnPanic:
						e.control.tooltip = dataManager.getString("Str_WarnPanic");
						break; // (패닉 - 0:정상, 1:패닉) : 패닉
					case TerminalStatusWarnCricis:
						e.control.tooltip = dataManager.getString("Str_WarnCricis");
...
function popupLogImage(index, imageData, temperature, temperatureValid) {
	...
	var path = "app/custom/hcsm/monitoring/hcsmMonitoringAuthImage" + "?" + usint_version;
	var selectionEvent = new cpr.events.CUIEvent("execute-menu", {
		content: {
...
function onMRMAN_grdTerminalListContextmenu( /* cpr.events.CMouseEvent */ e) {
	...
	terminalMenu.addEventListener("selection-change", function(e) {
		switch (terminalMenu.value) {
			case "0":
				processDoorCommand(0);
				break; //Str_DoorCommandOpenTemp				
			case "1":
				processDoorCommand(1);
				break; //Str_DoorCommandOpen				
			case "2":
				processDoorCommand(2);
				break; //Str_DoorCommandClose
			case "3":
				processSyncCommand();
				break; // 단말 동기화 
			...
			case "4":
				processAcuDoorCommand(0, 1);
				break;
			case "5":
				processAcuDoorCommand(0, 2);
				break;
			case "6":
				processAcuDoorCommand(0, 3);
				break;
			case "7":
				processAcuDoorCommand(0, 4);
				break;
				
			case "9":
				processAcuDoorCommand(1, 1);
				break;
			case "10":
				processAcuDoorCommand(1, 2);
				break;
			case "11":
				processAcuDoorCommand(1, 3);
				break;
			case "12":
				processAcuDoorCommand(1, 4);
				break;
				
			case "14":
				processAcuDoorCommand(2, 1);
				break;
			case "15":
				processAcuDoorCommand(2, 2);
				break;
			case "16":
				processAcuDoorCommand(2, 3);
				break;
			case "17":
				processAcuDoorCommand(2, 4);
				break;
				
			case "19":
				processAcuDoorCommand(3, 1);
				break;
			case "20":
				processAcuDoorCommand(3, 2);
				break;
			case "21":
				processAcuDoorCommand(3, 3);
				break;
			case "22":
				processAcuDoorCommand(3, 4);
				break;
				
			case "24":
				processAcuDoorCommand(4, 1);
				break;
			case "25":
				processAcuDoorCommand(4, 2);
				break;
			case "26":
				processAcuDoorCommand(4, 3);
				break;
			case "27":
				processAcuDoorCommand(4, 4);
				break;
				
			case "29":
				processShowAcuStatus();
				break;
			case "31":
				processResetEmergencyControl();
				break;
			case "32":
				terminalOptionSetting();
				break;
				
			case "33":
				processTerminalLockCommand(1);
				break; // 단말기 폐쇄
			case "34":
				processTerminalLockCommand(0);
				break; // 단말기 폐쇄 해제
...
function onMRMAN_btnDoorControlClick( /* cpr.events.CMouseEvent */ e) {
	...
	for (var i = 0; i < total; i++) {
		...
		if (row.getValue("Status") == 0 || row.getValue("Status") == 16) {
			continue;
		}
		dsTerminalIDList.addRowData({
			"ID": row.getValue("ID"),
			"RemoteDoor": row.getValue("RemoteDoor")
		});
	}
	...
	if (TMMGR_brandType == BRAND_VRIDI) {
		...
		switch (terminalType) {
			...
			case 22:
				...
				break;
			case 24:
				path = "app/main/terminals/eNCardi/OptionFrame";
				break;
			case 25:
				path = "app/main/terminals/T2/OptionFrame";
				break;
			case 26:
				path = "app/main/terminals/XSlim/OptionFrame";
				break;
			default:
				alert("undefined terminal type");
				return
		}
	}
	var selectionEvent = new cpr.events.CUIEvent("execute-menu", {
		content: {
			"Target": DLG_TERMINAL_INFO,
			"InitVal": {
				"ID": terminalRow.getRowData()["ID"],
				"Path": path
			}
		}
	});
...
function authlogListGridSetting() {
	app.getRootAppInstance().openDialog("app/custom/hcsm/monitoring/hcsmMonitoringGrid", {
		width: 570,
		height: 570
	}, function(dialog) {
		...
		app.lookup("MRMAN_grdAuthlog").addColumn({
			...
			header: [{
				...
				configurator: function(cell) {
					...
					cell.text = dataManager.getString("Str_" + strLang);
				}
			}],
			detail: [{
				constraint: {
				...
				configurator: function(cell) {
					...
					if (selectArr[i] == "BloodID") {
						cell.control = (function() {
							var comboBox_5 = new cpr.controls.ComboBox("MRMAN_cmbBloodType");
							comboBox_5.readOnly = true;
							(function(comboBox_1) {})(comboBox_5);
							comboBox_5.bind("value").toDataColumn("BloodID");
							comboBox_5.style.css("text-align", "center");
							return comboBox_5;
						})();
					}
					if (selectArr[i] == "Latitude") {
						cell.control = (function() {
							var comboBox_6 = new cpr.controls.ComboBox("MRMAN_cmbInOut");
							comboBox_6.readOnly = true;
							(function(comboBox_1) {})(comboBox_6);
							comboBox_6.bind("value").toDataColumn("Latitude");
							comboBox_6.style.css("text-align", "center");
							return comboBox_6;
						})();
					}
				}
