...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	var udcAuthLogList;
	udcAuthLogList = new udc.grid.authLogListIDIS("ALMGR_udcAuthLogList");		
...
function sendAuthLogListRequest() {
	...
	var smsGetAuthLogList = app.lookup("sms_getAuthLogCustomsIDIS")
	
	var cmbCategory = app.lookup("ALMGR_cmbCategory");
	var edtKeyword = app.lookup("ALMGR_edtKeyword");
	
	var StartWorkTime = app.lookup("TNAEWR_ipIntime");
	var OverWorkTime = app.lookup("ALMGR_WorkTime");
	...
	smsGetAuthLogList.setParameters("Keyword2", StartWorkTime.value);
	smsGetAuthLogList.setParameters("Keyword", OverWorkTime.value);
...
function getLogAuthTypeString( value ){
	...
	switch ( value ){
		...
		case 16: type = dataManager.getString("Str_NotAssigned"); break;
 ...
 * 응답처리가 모두 종료되면 발생합니다.
 */
function onSms_getAuthLogCustomsIDISSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	/** 
	 * @type cpr.protocols.Submission
	 */
	var sms_getAuthLogCustomsIDIS = e.control;
	
	var temperatureUnit = dataManager.getTemperatureUnit();
		
	var resultCode = app.lookup("Result").getValue("ResultCode");
	if (resultCode == COMERROR_NONE) {
		var Totalcnt = app.lookup("Total").getValue("Count");
		var dsAuthLogList = app.lookup("WCAuthLogIDIS");
		var count = dsAuthLogList.getRowCount();
		var terminalList = dataManager.getTerminalList();
		for(var i =0; i<count; i++){
			var logInfo = dsAuthLogList.getRow(i);
			...
			// -- idis 출근/지각/결근 추가 
			var WorkResult;
			if (logInfo.getValue("WorkResult") == 0) {
				WorkResult = dataManager.getString("Str_TaskStatePending")+ " ";
			} else if (logInfo.getValue("WorkResult") == 1) {
				WorkResult = dataManager.getString("Str_TNAIn")+ " ";
			} else if (logInfo.getValue("WorkResult") == 2) {
				WorkResult = dataManager.getString("Str_TNALate")+ " ";
			} else if (logInfo.getValue("WorkResult") == 3) {
				WorkResult = dataManager.getString("Str_TNAAbsence")+ " ";
			} else {
				WorkResult = "--"
			}	
			logInfo.setValue("WorkResult", WorkResult)
		}
		var dmTotal = app.lookup("Total");
		var totalCount = parseInt(dmTotal.getValue("Count"));
