...
//	var appwidth = 3840;
	...
	var storage_Image = localStorage.getItem("displayBoard_Iamge");
	var storage_Name = localStorage.getItem("displayBoard_Name");
	var storage_Count = localStorage.getItem("displayBoard_Count");
	var storage_FontSize = localStorage.getItem("displayBoard_FontSize");
	var cmbTerminalList = localStorage.getItem("cmbTerminalList");

	if (cmbTerminalList != null && cmbTerminalList != "null") {
		app.lookup("MDB_cmbTerminalList1").value = cmbTerminalList;
		onMDB_cmbTerminalList1ItemClick();
	}
	
	if (storage_Image != null && storage_Image != "") {
		app.lookup(dm_returnValue.setValue("image", storage_Image));
	} else {
		storage_Image = '../../../theme/images/monitoring/MonitoringDisplayBoard_bi3.jpg';
	}
	...
	if (storage_FontSize != null && storage_FontSize != "") {
		dm_returnValue.setValue("font_size", storage_FontSize);
	} else {
		storage_FontSize = "35";
		dm_returnValue.setValue("font_size", storage_FontSize);
	}
	...
	app.lookup("MDB_itone_logo").visible = true;
	
	
	// 로고 이름 사이즈 테스트 중
	app.lookup("MDB_itone_logo").style.css({
		"font-size" : storage_FontSize +"px",
	})

	var layoutSize = Number(storage_FontSize)*1.3;
	app.lookup("MDB_BackgroundGroup").getLayout().setRows([layoutSize+"px","5fr"]);
	
	var backgroundLayout = app.lookup("MDB_BackgroundGroup");
	backgroundLayout.style.css({
		"background-image" : "URL("+storage_Image+")",
		"background-size" : "100% 100%",
	});
	
	backgroundLayout.redraw();
	
	
	
	// 일반향에서 사용하는 컨트롤 제거
	app.lookup("MDB_normal_logo").visible = false;
	app.lookup("ID상호명").visible = false;
	
}

// 웹 소켓 수신 시 레이아웃 생성
exports.addAuthLog = function(authLog) {
	// 인증없는 상태면 레이아웃 클리어. 인증이 있으면 레이아웃 보이도록
	TimerUpdate();
	...
	if (selectTerminalListValues.length == 0) {
		...
		return;
	} else if (authLog.AuthResult != "0") {
		console.log('인증 실패는 안보여주도록 요청');
		return;	
	}
	...
	if (selectTabItem == 1) {
		...
		if (selectTerminalListValues.includes(authLog.TerminalID.toString())) {
			...
			// 추가된 레이아웃에 바인딩
			var udcList = new udc.custom.AuthDisplayBoardITONE("USMAG_udcUserList");
...
function terminalSelectTab1() {
	...
	var sideSpacing = (Math.floor(appwidth / 10)).toString() + "px";
//	columnSize = (Math.floor(appwidth / 4) - Math.floor(appwidth / 430)).toString() + "px";
	columnSize = (Math.floor(appwidth / 5)).toString() + "px";
	
	// 세로순으로 row 생성
	authGroup1.getLayout().insertRows(["1fr"]);
	
	// 홀수번째 선택은 윗줄, 짝수번째 선택은 아랫줄 (위 : 출근, 아래 : 퇴근 단말)
	authGroupLayout.insertColumns(['1fr']);
	for (var count=1; count <= cmbTerminalList1Length; count++) {
		// 홀 수 번째마다 컬럼 추가
		if (count % 2 == 1) {
			authGroupLayout.insertColumns([columnSize]);
			
		} 
		
		// USMAG_udcUserList + 선택한 단말기 ID 
		var udcName = "USMAG_udcUserList";
		var selectTerminalId = cmbTerminalList1.values[count-1];
		// udc
		var udcList = new udc.custom.AuthDisplayBoardITONE(udcName + selectTerminalId);
		
		// 적용 클릭 시 단말기명 set
		var terminalName = dataManager.getTerminalList().findFirstRow("ID == '" + cmbTerminalList1.values[count-1] + "'").getValue('Name');
		udcList.setTerminalName(terminalName);
		
		var rowIndex = 0;
		var colIndex = Math.floor(((count+1) / 2));
...
function onMDB_cmbTerminalList1ItemClick( /* cpr.events.CItemEvent */ e) {
	...
	if (cmbTerminalList1.values.length >= 9) {
		...
		cmbTerminalList1.removeSelection(cmbTerminalList1.getSelectionLast(), false);
		dialogAlert(app, dataManager.getString("Str_Failed"), "8개까지 선택할 수 있습니다.");
	} else {
		// 1탭 단말기 선택 적용
		localStorage.setItem("cmbTerminalList", cmbTerminalList1.value);
		terminalSelectTab1();
	}
}
...
function onImageClick4( /* cpr.events.CMouseEvent */ e) {
	...
	var dm_returnValue = app.lookup('returnValue');
	rootApp.openDialog("app/custom/itone/displayBoardMultiLangCustom", option, function( /*cpr.controls.Dialog*/ dialog) {}).then(function(returnValue) {
		...
		localStorage.setItem("displayBoard_FontSize", dm_returnValue.getValue("font_size"));
		localStorage.setItem("displayBoard_PopUpCount", dm_returnValue.getValue("popup_count"));
		...
		// 로고 이름 사이즈 변경
		var fontSize = dm_returnValue.getValue("font_size");
		app.lookup("MDB_itone_logo").style.css({
			"font-size" : fontSize +"px",
		})
		
		var layoutSize = Number(fontSize)*1.3;
		
		app.lookup("MDB_BackgroundGroup").getLayout().setRows([layoutSize+"px","5fr"]);
		
		// 배경 이미지 변경
		var storage_Iamge = dm_returnValue.getValue("image");
		if(storage_Iamge == "") {
			storage_Iamge = '../../../theme/images/monitoring/MonitoringDisplayBoard_bi3.jpg';
		}
		app.lookup("MDB_itone_logo").visible = true;
		var backgroundLayout = app.lookup("MDB_BackgroundGroup");
		backgroundLayout.style.css({
			"background-image" : "URL("+storage_Iamge+")",
			"background-size" : "100% 100%",
		});
 ...
 */
function onTest_layoutMouseenter(/* cpr.events.CMouseEvent */ e){
	/** 
	 * @type cpr.controls.Container
	 */
	var test_layout = e.control;
	app.lookup("MDB_terminalSelectgroup1").visible = true;
}
 ...
 */
function onTest_layoutMouseleave(/* cpr.events.CMouseEvent */ e){
	/** 
	 * @type cpr.controls.Container
	 */
	var test_layout = e.control;
	if(app.lookup("MDB_cmbTerminalList1").focused) {
		return;
	}
	app.lookup("MDB_terminalSelectgroup1").visible = false;
}


function TimerUpdate() {
	var wrappingGroup = app.lookup("authGroupWrapping");
	wrappingGroup.visible = true;
	
	var popup_Count = localStorage.getItem("displayBoard_PopUpCount");
	if (popup_Count != null && popup_Count != "") {
		timeCount = parseInt(popup_Count, 10) * 1000;
	}
	
	if (timeCount != 0) {
		startTimer();
//		app.lookup("authGroupWrapping").visible = false;
		function startTimer() {
			clearTimeout(timer); // 기존 타이머 취소
			timer = setTimeout(function() {
				wrappingGroup.visible = false;
			}, timeCount);
		}
	}
}