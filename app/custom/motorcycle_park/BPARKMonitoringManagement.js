...
function onBodyLoad( /* cpr.events.CEvent */ e) {
	...
	if (terminalList) {
		...
		if (Loginuserid != 1000000000000000000 && LoginPrivilege != 1) { // Master 가 아니기 때문에 그룹 제외 처리 - otk
			...
			console.log("[else] terminalid : " + terminalList.getValue(i, "ID"));
...
function initComboEventLogContents() {
	...
	cmbEventContent.addItem(new cpr.controls.Item(dataManager.getString("Str_BPARK_AlpetaServerDisconnected"), 393235));					// 알페타 연결 끊김    근데 요거는 연결 끊켜서 못받음
...
//		console.log("changedValue : " + changedValue);
		...
		var statusBinary = changedValue.toString(2).padStart(32, "0");	// 바껴서 들어오는거
		var valueBinary = msg[i].Status.toString(2).padStart(32, "0");  // 바까줄거
		
//		console.log("statusBinary : "+ statusBinary);
//		console.log("valueBinary : " + valueBinary);
		
		// 베트남 주차관제 단말기 경우 lpr 경우 분기
//		if ( msg[i].Type == "1000" ) {	// lpr 장비일 경우
...
//			if(changedValue==0&&force==false){return;}
			...
			// LPR 보드
			// status1	 알페타 서버 연결
			// status2	 메인함체 , 서브함체 컨트롤보드 연결
			// status3	메인함체, 서브함체 카메라 연결
			// 카메라 연결 확인 튤팁으로 
			// status4	LPR 모듈 (SW) 연결
//		} else {
...
function onStatusUpdate(category, value, terminal){
	...
	switch( category ){
		...
		// 아직 숫자는 미정
		// 베트남 주차관제 에러코드
		case 25:	// LPR 보드 <-> 메인함체 컨트롤보드 연결
			if ( value == 1 ){ /* 미정 */  }
			break;
		case 26:	// LPR 보드 <-> 서브함체 컨트롤보드 연결
			if ( value == 1 ){ /* 미정 */  }
			break;
		case 27:	// LPR 보드 <-> LPR 모듈 (SW) 연결
			if ( value == 1 ){ /* 미정 */  }
			break;
		case 28:	// LPR 보드 <-> 메인함체 카메라 연결
			if ( value == 1 ){ /* 미정 */  }
			break;
		case 29:	// LPR 보드 <-> 서브함체 카메라 연결
			if ( value == 1 ){ /* 미정 */  }
			break;
		case 30:	// LPR 보드 <-> 알페타 서버 연결
			if ( value == 1 ){ /* 미정 */  }
			break;
	}
}
...
function onMRMAN_btnDoorControlClick(/* cpr.events.CMouseEvent */ e){
	...
	if (dataManager.getOemVersion() == OEM_MOTORCYCLE_PARK) {
		...
		if ( cmbDoorControlOption.value == 100 ) {	// lpr 열림일 경우
		
			for( var i = 0; i < total; i++ ) {
				var row = grdTerminalList.getRow(chkIndices[i]);
				//if( row.getValue("Type") != 1000 ){continue;} // lpr type 이 아닌 경우 패스
			...
			for( var i = 0; i < total; i++ ){
				var row = grdTerminalList.getRow(chkIndices[i]);
				if( (row.getValue("Status") == 0 || row.getValue("Status") == 16) || row.getValue("Type") == 1000 ){continue;} // lpr type 인 경우 패스
				dsTerminalIDList.addRowData({"ID":row.getValue("ID"), "RemoteDoor":row.getValue("RemoteDoor")});
			}
		}
		...
		}else if(cmbDoorControlOption.value == 100) {	// lpr 열림
			sendLprOpenCommandBPARK();
		}else{
...
function sendLprOpenCommandBPARK() {
	// lpr 열기로 빠지는 경우
	if (dataManager.getOemVersion() != OEM_MOTORCYCLE_PARK) {
		return
	}
	...
	if( row ){
		...
		dmDoorOption.setValue("ID", terminalID);
		dmDoorOption.setValue("Option", doorCommand);
//		console.log("id : " + terminalID+ "  Option : " + doorCommand);
		var dmResult = app.lookup("Result");
		var sms_postTerminalDoorControl =  new cpr.protocols.Submission("sms_postTerminalDoorControl");
		sms_postTerminalDoorControl.action = "v1/bparkLpr/" + terminalID + "/control/door";		
		sms_postTerminalDoorControl.method = "post";
		sms_postTerminalDoorControl.addRequestData(dmDoorOption);				
		sms_postTerminalDoorControl.addResponseData(dmResult);
		
		sms_postTerminalDoorControl.addEventListenerOnce("submit-done", onSms_postBPARKLprControlExDone);
		sms_postTerminalDoorControl.addEventListenerOnce("submit-error", onSms_postTerminalDoorControlExError);
		sms_postTerminalDoorControl.addEventListenerOnce("submit-timeout", onSms_postTerminalDoorControlExTimeout);
...
function onSms_postBPARKLprControlExDone( /* cpr.events.CSubmissionEvent */ e) {
	comLib.updateLoadMask();
	console.log("onSms_postBPARKLprControlExDone");
	sendLprOpenCommandBPARK();
}
