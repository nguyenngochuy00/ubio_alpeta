...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	app.lookup("MMZMP_dtSearchDate").value = today;
...
function onMMZMP_btnZoneAddClick(/* cpr.events.CMouseEvent */ e){
	console.log("onMMZMP_btnZoneAddClick !!");
...
function sendMusteringTerminalDelete(){
	...
	sms_deleteMusteringTerminal.addEventListenerOnce("submit-error", onSubmitError);
	sms_deleteMusteringTerminal.addEventListenerOnce("submit-timeout", onSubmitTimeout);
...
/*
 * "조회" 버튼(btSearch)에서 click 이벤트 발생 시 호출.
 * 사용자가 컨트롤을 클릭할 때 발생하는 이벤트.
 */
function onBtSearchClick(/* cpr.events.CMouseEvent */ e){
	var btSearch = e.control;

	app.lookup("RollCallList").clear();
	var rowIndex = app.lookup("MMZMP_grdMusteringSearch").getSelectedRowIndex();
	if(rowIndex < 0){
		dialogAlert(app, dataManager.getString("Str_Warning"), dataManager.getString("Str_NoSelectionZone"));
	}
	var rowData = app.lookup("MusteringList").getRow(rowIndex);
	
	var submision = app.lookup("sms_getRollcallInfosNDPP");
	submision.setParameters("searchDate", app.lookup("MMZMP_dtSearchDate").value);
	submision.setParameters("musteringID", rowData.getValue("MusteringID"));
 ...
 * 응답처리가 모두 종료되면 발생합니다.
 */
function onSms_getRollcallInfosNDPPSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	console.log(app.lookup("RollCallList").getRowDataRanged());
	console.log(app.lookup("Total").getDatas());
	
	var resultCode = app.lookup("Result").getValue("ResultCode"); 
	if( resultCode == 0 ){
		var today = dateLib.getToday("-");
		var dsRollCall = app.lookup("RollCallList");
		
		if (dsRollCall.getRowCount() == 0) {
			dialogAlert(app, dataManager.getString("Str_Success"), dataManager.getString("Str_NoSearchResult"));
		} else {
			for (var i=0 ; i < dsRollCall.getRowCount(); i++) {
				var row = dsRollCall.getRow(i);
				if (row.getValue("InCount") == 0 && row.getValue("OutCount") != 0) {
					row.setValue("RollCallStatus", -1);
				} else if (row.getValue("InCount") != 0 && row.getValue("OutCount") == 0) {
					if (today == app.lookup("MMZMP_dtSearchDate").value) {
						row.setValue("RollCallStatus", 1);	
					} else {
						row.setValue("RollCallStatus", -1);
					}
				} else {
					row.setValue("RollCallStatus", 0);
				}
				row.setState(cpr.data.tabledata.RowState.UNCHANGED);
			}
		}
		app.lookup("MMZMP_opbTotal").redraw();
	}else{
		dialogAlert(app, dataManager.getString("Str_Failed"), dataManager.getString(getErrorString(resultCode)));		
	}	
}
