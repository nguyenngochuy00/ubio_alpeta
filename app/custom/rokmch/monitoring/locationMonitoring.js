...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	smsGetAreaList.addEventListenerOnce("submit-success", function(){
		...
		// 장비 위치 추가  - pse
		var cmbTerminalLocation = app.lookup("VMMON__cmbTerminalLocation");
		if (targetDataSet.getRowCount() > 0) {
			var i;
			for (i = 0; i < targetDataSet.getRowCount(); i++) {
				//console.log(dsTerminalList.getValue(i, "ID") + ", " + dsTerminalList.getValue(i, "Name"));
				cmbTerminalLocation.addItem(new cpr.controls.Item(targetDataSet.getValue(i, "Name"), targetDataSet.getValue(i, "ID")));
			}
		}
	});
}
...
function onSms_getMapAreaTerminalListSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	...
	if(result==0){
		...
		console.log(mapAreaList.getRowDataRanged());
...
function onMRMAN_cmbUserTypeSelectionChange(/* cpr.events.CSelectionEvent */ e){
	var userType = app.lookup("cmbUserType").value;	
