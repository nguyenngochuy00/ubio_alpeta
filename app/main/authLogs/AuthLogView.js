...
//영상로그 관련
//var devSerial; // vurix에 등록돤 장비 번호
var videoFlag = 0; // 0: 둘다 없음,  1: IDIS,   2: Vurix
...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	var paramTerminalList = initValue["Param"][2].dsTerminalList;
	var terminalList = app.lookup("TerminalList");
	paramTerminalList.copyToDataSet(terminalList);
	var paramConfig = initValue["Param"][2].vurixConfig;
	var vurixConfig = app.lookup("Config");
	paramConfig.copyToDataMap(vurixConfig);
...
function onSms_getAuthLogListDetailSubmitDone(/* cpr.events.CSubmissionEvent */ e){	
	...
	if( resultCode == COMERROR_NONE){ // 로그인 상태
		...
		var row = terminalList.findFirstRow("ID == " + terminalID)
		if(row) {
			var devSerial = row.getValue("dev_serial");	
			var dmVideoInfo = app.lookup("VideoInfo");
			if(devSerial != '') {
				dmVideoInfo.setValue("devSerial", devSerial);
				dmVideoInfo.setValue("terminalID", terminalID);
				dmVideoInfo.setValue("terminalName", row.getValue("Name"));
				dmVideoInfo.setValue("eventTime", dmAuthLogDetail.getValue("EventTime"));
				videoFlag = VMSInnodep; // Vurix에 등록된 단말기
			}
		}
...
function displayAuthLogDetail() {
	...
	// 사용자 사진 image 컨트롤
	var userPicture = app.lookup("user_image");
	
	// 인증로그 사진
	var logImage = dmAuthLogDetail.getValue("LogImage");
	
	// 인증로그 사진  image 컨트롤
	var logPicture = app.lookup("log_image");

	// DMCC 마스터 로그인 시 에만 사용자 사진 보이도록 
	switch (oem_version) {
		case OEM_DMCC_NOPICTURE:
			/* 주석 풀면 마스터만 볼 수 있음
			// 사용자 사진
			if (userImage.length > 0 && Number(account.getValue("UserID")) == 1000000000000000000) {
				userPicture.src = "data:image/png;base64," + dmAuthLogDetail.getValue("UserImage");
			} else {
				userPicture.src = "../../../theme/images/noImg.gif";
			}
			
			// 로그 사진
			if (logImage.length > 0 && Number(account.getValue("UserID")) == 1000000000000000000) {
				logPicture.src = "data:image/png;base64," + dmAuthLogDetail.getValue("LogImage");
			} else {
				logPicture.src = "../../../theme/images/noImg.gif";
			}
			*/
			break;
...
function onButtonClick3(/* cpr.events.CMouseEvent */ e){
	switch(videoFlag){
		...
		case VMSInnodep:
			eventViewAPI();
//			InnodepVideoLog(); // 추후에 DMS써서 EventView 본다면 eventViewAPI를 주석처리 후 주석해제
			break;
		default :
			break;
	}
...
// 주차관제 시스템 이미지 확대 기능 추가 - otk
function onLog_imageClick(/* cpr.events.CMouseEvent */ e){
	oem_version = dataManager.getOemVersion();
	if(oem_version == OEM_MOTORCYCLE_PARK){
		var dmAuthLogDetail = app.lookup("AuthLogDetail");
		var Index = dmAuthLogDetail.getValue("IndexKey");	
		var selectionEvent = new cpr.events.CUIEvent("execute-menu", {
			content: {
				"Target":DLG_AUTHLOGDETAILIMAGE,
				"Index":Index,
			}
		});
		app.getHostAppInstance().dispatchEvent(selectionEvent);
...
function videoViewVisible(){
	var licenseLevel = dataManager.getSystemLicenseLevel();
	
	// Standard 이상 일때만 EventView 사용 -mjy
	if(videoFlag > 0 && licenseLevel >= LicenseSTANDARD) {
		app.lookup("ALVEW_btnAuthLogVideoView").visible = true;
	} else {
		app.lookup("ALVEW_btnAuthLogVideoView").visible = false;
	}
...
function InnodepVideoLog(){
	var dmVideoInfo = app.lookup("VideoInfo");
	var event_time = dmVideoInfo.getValue("eventTime"); // format : '2023-09-08 15:43:37'
	var vurixConfig = app.lookup("Config");
	var unixTime = Number(moment(new Date(event_time), 'YYYY-MM-DDTHH:mm').unix());
	
	var authResult = app.lookup("AuthLogDetail").getValue("AuthResult");
	var dmPlayTime = app.lookup("PlayTime");
	
	var fileName = app.lookup("ResultMessage").getValue("Message");
	

	var appID = "app/main/authLogs/authLogVideoViewVurix";
	...
	app.openDialog(appID, dialogProp, function(dialog) {
		dialog.style.animateFrom({	
			"transform": "translateX(100%)",
			"opacity": "0"
		}, .5, cpr.animation.TimingFunction.EASE_IN_OUT);
		dialog.ready(function(dialogApp){
			dialog.headerTitle = dataManager.getString("Str_AuthLogVideoView");
		});
		dialog.addEventListenerOnce("init", function () {
			dialog.setAppProperties({
				"UnixTime": unixTime,
				"VurixConfig": vurixConfig,
				"VideoInfo": dmVideoInfo,
 ...
 * 응답처리가 모두 종료되면 발생합니다.
 */
function onSms_downloadEventViewSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	/** 
	 * @type cpr.protocols.Submission
	 */
	comLib.hideLoadMask();
...
function eventViewAPI() {
	var dmVideoInfo = app.lookup("VideoInfo");
	var event_time = dmVideoInfo.getValue("eventTime"); // format : '2023-09-08 15:43:37'

	// EventView를  RTSP가 아닌 API로 다운로드해서 보여주도록 - mjy --------------------------------------
	setEventViewDownload();
	
	comLib.showLoadMask("", "", "", 0);
	app.lookup("sms_getEventView").send();
}


// API는 1분단위로 동영상을 가져오는데 0초에 인증 혹은 59초에 인증했을 경우, 인증 이전 상황 or 인증 이후 상황을 볼 수 없으므로 2분을 땡겨와야해서 생성
function setEventViewDownload() {
	if(dataManager.getSystemLicenseLevel() == LicenseSTANDARD) {
		var dmVideoInfo = app.lookup("VideoInfo");
		var dev_serial = dmVideoInfo.getValue("devSerial");
		var terminalID = dmVideoInfo.getValue("terminalID");
		
		var event_time = dmVideoInfo.getValue("eventTime"); // format : '2023-09-08 15:43:37'
		var FromDate = new Date(event_time); // 날짜 계산용
		var ToDate = new Date(event_time); // 날짜 계산용
		
		var dmPlayTime = app.lookup("PlayTime");
		dmPlayTime.setValue("startSecond", getSeconds(FromDate));
		
		var from_date = "";
		var to_date = "";
		
		var aa = event_time.replaceAll("-","").split(" ");
		var hhmmssArray = aa[1].split(":"); // 0: hh , 1:mm , 2:ss
		var ss = parseInt(hhmmssArray[2]); // 초단위
	
		// 2분 땡겨와야하는 경우 분기처리.	
		if(ss < 3) {
			// 3초 미만이면 기존보다 1분 앞당긴다. (총 2분)
			FromDate.setMinutes(FromDate.getMinutes()-1);
			from_date = getYYYYmmddHHmm(FromDate);
			to_date = getYYYYmmddHHmm(ToDate);
			
		} else if(ss > 57) { 
			// 57초보다 크면 뒤에 1분 더 가져온다. (총 2분)
			from_date = getYYYYmmddHHmm(FromDate);
			...
			// 일반적인 상황 (1분)
			from_date = getYYYYmmddHHmm(FromDate);
			to_date = getYYYYmmddHHmm(FromDate);
		}
		
		dmPlayTime.setValue("FromDate", from_date); // 동영상에 오버레이 띄워주는 정보로써 사용
		
		var dmEVD = app.lookup("EventViewDownload"); // 서브미션 보낼 때 사용
		dmEVD.setValue("FromDate", from_date);
		dmEVD.setValue("ToDate", to_date);
		dmEVD.setValue("DevSerial", dev_serial);
		dmEVD.setValue("TerminalID", terminalID);
	}
}

// 날짜 게산 후 다시 string으로 변환
function getYYYYmmddHHmm(date) {
	...
	var s1 = yyyy+mm+dd+hh+m;
	return s1;
}

function getSeconds(date) {
	var ss = parseInt(date.getSeconds().toString());
	return ss;
}
...
function onBodyBeforeUnload(/* cpr.events.CEvent */ e){
	if(videoFlag == VMSInnodep) {
		var deleteEventView = app.lookup("sms_deleteEventView");
		deleteEventView.action = "/v1/vurix/eventViewDelete" + "/EventView";
