...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	var adminID = dataManager.getAccountID();
	var UserInfo= app.lookup("UserPasswordInfo");
	UserInfo.setValue("ID", Number(adminID));
...
function onADPWU_btnPassWordUpdateClick (/* cpr.events.CMouseEvent */ e) {
	...
	case OEM_ARMY_HQ:
		if ( NewPassword.length < 9 || ComfirmPassword.length < 9  ) {
			dialogAlert(app, dataManager.getString("Str_Warning"), "비밀번호는 최소 9자리 이상 입력해야 합니다.");
			return;
		}
		break;
	default:
	...
	if (initValue) {
		if (initValue["VisitFlag"]==1) {
			RequestData.action = "/v1/visit/users/" + UserInfo.getValue("ID") + "/loginpassword";
		}
		// method 우회
		if (oem_version == OEM_HYUNDAI_MSEAT) {
			RequestData.method = "POST";
			RequestData.action += "HDMSPUT";
		}
	}
...
//	app.close(1);
		dialogAlert(app.getRootAppInstance(), dataManager.getString("Str_PasswordChangeSuccess"), dataManager.getString("Str_Success"), function( /*cpr.controls.Dialog*/ dialog) {
