...
function onSms_FullImageReqSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	 ...
	 */
	var sms_FullImageReq = e.control;

	var resultInfo = app.lookup("Result");
	var result = resultInfo.getValue("ResultCode");
	console.log("onSms_FullImageReqSubmitDone result = ", result);
...
function onSms_getMapAreaListSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	...
	var resultInfo = app.lookup("Result");
	var result = resultInfo.getValue("ResultCode");
	console.log("onSms_getMapAreaListSubmitDone result = ", result);
...
function onSms_updateMpaAreaPositionSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	...
	if(result==0){
		...
		//dialogAlert(app, "", dataManager.getString(""), "");
		dialogAlert(app, dataManager.getString("Str_Warning"), dataManager.getString(getErrorString(result)), "");
	}
}
...
function onBtnAreaSelectClick(/* cpr.events.CMouseEvent */ e){
	...
	if(!vMMGR_imgFull.src){
		...
		dialogAlert(app, dataManager.getString("Str_Warning"), dataManager.getString("Str_ErrorAfterImageRegist"), "");
		return;
	}
	...
	var result = [];
	var appld = "app/main/mapDrawing/locationRegistArea" + "?" + usint_version;
...
function createDragIcon(id, name, left, top){
	...
	dragContainer.addEventListener("contextmenu", function(e){
		...
		menu_1.addEventListener("blur", function(e) { menu_1.dispose();	});
