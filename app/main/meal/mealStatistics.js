...
var MSSTA_pageInit = new Array(1, 1, 0, 0, 0, 0); // 페이지 초기화 여부 변수. 더미, 월간 통계, 식수 집계, 그룹별 조회, 개인별 조회
...
function sendGetMealResultList() {
	...
	var code = app.lookup("MSSTA3_cmdSearchmeal");
	var mealdatacode = code.text.substr(0, 4);
	if (mealdatacode == "----"){
		mealdatacode = "";
	}
...
function onSms_getMealResultListSubmitDone( /* cpr.events.CSubmissionEvent */ e) {
	...
	if (dmResult.getValue("ResultCode") == COMERROR_NONE) {
		...
		var Mealdatacode = app.lookup("MSSTA3_cmdSearchmeal");		
		...
		if (dmExportParam.getValue("mode") !== "export") {
			...
			// -----> 식수 조회 시 끼니코드와 종류가 맞지 않는 증상 수정 -  220524 otk
			for(var i=0; i < totalCount; i++)
			{
				var CurCol = dsMealResult.getRow(i);
				var type = CurCol.getValue("Type");
				var nType = parseInt(type, 10);			
				
				for(var j=0; j <= 5; j++)
				{
					var code = Mealdatacode.getItem(j).label;
					if (j == 0){
						continue;
					}
					mealdatacode = code.split(":")[0];
					var mealcode = parseInt(mealdatacode);
					var mealdataName = code.split(":")[1].trim();
					if(nType == mealcode)
					{
						MealdateCdName = mealdataName;
						break;
					}
				}
				MealResultGrid.addItem(new cpr.controls.Item(MealdateCdName, mealcode));
			}
			// <----- 식수 조회 시 끼니코드와 종류가 맞지 않는 증상 수정 -  220524 otk
...
function onSms_getMealDataListSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	...
	var cmbMealData = app.lookup("MSSTA3_cmdSearchmeal");
