...
function onBodyLoad( /* cpr.events.CEvent */ e) {
	...
	if (terminalList) {
		...
		// 그룹 Admin으로 로그인 시 단말기가 Logon/Logoff 됬을 시 해당 단말기의 groupcode가 Null이 되는 증상 수정  - otk 
		...
		if (dataManager.getOemVersion() != OEM_KANGWONLAND ) {
			if (Loginuserid != 1000000000000000000 && LoginPrivilege != 1) { // Master 가 아니기 때문에 그룹 제외 처리 - otk
				if (groupList) {
					for (var i = 0; i < terminalList.getRowCount(); i++) {
						var terminalInfo = terminalList.getRow(i);
						var terminalgroupcd = terminalInfo.getValue("GroupCode");
						console.log("group code : ", terminalgroupcd);	
						console.log("group count : ", terminalList.getRowCount());
						console.log("terminalid : " + terminalInfo.getValue(i, "ID"));
						if (terminalgroupcd == 0) {
							continue;
						} else {
							console.log("terminalid : " + terminalList.getValue(i, "ID"));
							var rowData = {
								ID : terminalList.getValue(i, "ID"),
								Name :terminalList.getValue(i, "Name"),
								Status : terminalList.getValue(i, "Status"),
								StatusEx : terminalList.getValue(i, "StatusEx"),
								Event : terminalList.getValue(i, "Event"),
								GroupCode : terminalList.getValue(i, "GroupCode"),
								TimezoneVersion : terminalList.getValue(i, "TimezoneVersion"),
								TerminalStatus1 : terminalList.getValue(i, "TerminalStatus1"),
								TerminalStatus2 : terminalList.getValue(i, "TerminalStatus2"),
								TerminalStatus3 : terminalList.getValue(i, "TerminalStatus3"),
								TerminalStatus4 : terminalList.getValue(i, "TerminalStatus4"),
								Type : terminalList.getValue(i, "Type"),
								RemoteDoor : terminalList.getValue(i, "RemoteDoor"),				
							}
							dsTerminalList.addRowData(rowData);
						}
					}
				}
			} else {
				var IDs = [];
				for (var i = 0; i < terminalList.getRowCount(); i++) { // console에 단말기 ID 로그 출력
					IDs[i] = terminalList.getValue(i, "ID");
				}
				console.log("[else] terminal ID : " + IDs);
		...
		//console.log(terminalList.getRowDataRanged());
	...
	app.lookup("MRMAN_treGroup").selectItem(0, true);
	
	if (dataManager.getOemVersion() == OEM_BOSK_CAPS) {
		var btnBoskFireAlarmClear = app.lookup("MRMAN_btnBoskFireAlarmClear");
		btnBoskFireAlarmClear.visible = true;
		btnBoskFireAlarmClear.enabled = true;
	}
}
...
function initComboEventLogContents() {
	...
	if (dataManager.getOemVersion() == OEM_BOSK_CAPS ) {
		cmbEventContent.addItem(new cpr.controls.Item("Door2 Open", 251658273));
		cmbEventContent.addItem(new cpr.controls.Item("Door2 Forced", 251658274));
		cmbEventContent.addItem(new cpr.controls.Item("Door2 NotClosed", 251658275));
		cmbEventContent.addItem(new cpr.controls.Item("Door2 NotMonitor", 251658276));
		cmbEventContent.addItem(new cpr.controls.Item("Door2 Closed", 251658277));
...
exports.updateTerminalInfo = function( terminalInfo, type ){
	...
	if( type == 1 || type == 2 ){ // type : 1 추가, 2 : 수정
		if( terminalRow ){
			...
			// 출입문 원격 제어 수정
			if (type == 2){
				var terminalListFindRow = dsTerminalList.findFirstRow('ID=='+terminalInfo.ID);
				if(terminalInfo.RemoteDoor != terminalListFindRow.getValue("remoteDoor")) {
					terminalListFindRow.setValue("RemoteDoor", terminalInfo.RemoteDoor);
				}
			}
...
exports.addAuthLog = function(authLog) {
	...
	// 인증기록 - '단말기명' 추가
	var terminalList = dataManager.getTerminalList();
	var terminalName = terminalList.findFirstRow("ID==" + authLog.TerminalID).getValue("Name");
	insertRow.setValue('TerminalName', terminalName);
	...
	if( authLog.LogImage ){ 
	 	...
	 	if( dataManager.getOemVersion() == OEM_ALMARAI_AUTHINFO) {
	 		var AuthLogImageInfo = app.lookup("AuthLogImageInfo");
	 		AuthLogImageInfo.setValue('LogIndex', -1 );
	 		AuthLogImageInfo.setValue('TerminalID', authLog.TerminalID);
	 		AuthLogImageInfo.setValue('TerminalName', terminalName);
	 		AuthLogImageInfo.setValue('UserId', authLog.UserID);
	 		AuthLogImageInfo.setValue('UserName', authLog.UserName);
	 		AuthLogImageInfo.setValue('AuthResult', authLog.AuthResult);
	 		AuthLogImageInfo.setValue('EventTime', authLog.EventTime);
	 		AuthLogImageInfo.setValue('LogImage', authLog.LogImage);
	 		popupLogImage_Almarai_authinfo(AuthLogImageInfo);	
	 	} else {
...
function onMRMAN_treGroupSelectionChange( /* cpr.events.CSelectionEvent */ e) {
	...
	for (var i = 0; grdTerminalList.getRowCount() > i; i++) {
		// 속도 개선을 위한 index번호 추가
		grdTerminalList.setValue(i, 'Rn', i);
	}
...
function processAcuDoorCommand(opt, door /* 1~4 */){
	...
	if (terminalRow) {
		...
		dmAcuDoorOption.setValue("Option", opt);
			...
			if(e.rowIndex == i && e.cellIndex == 4){
				...
				//console.log(row.getValue("TerminalStatus2"));
    				...
    				case TerminalStatusDoorEmergency: 
    					e.control.tooltip = dataManager.getString("Str_DoorEmergency"); break; //(침입 - 0:정상, 1:침입) : 강제 침입 
...
function popupLogImage( index, imageData, temperature, temperatureValid ){
	...
	if (oemVersion == OEM_VIETNAM_INTEG_CONTROL) {// 해당 버전에서는 안뜨도록 처리
		return;
	}
...
function popupLogImage_Almarai_authinfo(AuthLogImageInfo) {
	console.log("popupLogImage_Almarai_authinfo");
	var AuthLogImagePopup = dataManager.getClientOption().getValue("AuthLogImagePopup");
	if (AuthLogImagePopup == 0 ) {
		return;
	}
	
	var dlgMonitoringAuthImage = programManager.getProcess(DLG_ALMARAI_AUTHLOG_IMAGE,DLG_ALMARAI_AUTHLOG_IMAGE);
    if(dlgMonitoringAuthImage ){
		dlgMonitoringAuthImage.ctrl.close();
	}	
	var dmAuthLogImage = app.lookup("AuthLogImageInfo");		
	var Almarai_AuthImageLeft = 50;
	var Almarai_AuthImagetop = 100;
	var Almarai_AuthImageWidth = 300; 
	var Almarai_AuthImageHeight = 410;
	
	var menu_id = DLG_ALMARAI_AUTHLOG_IMAGE;
	var path = "app/custom/almarai/MonitoringPopupImage" + "?" + usint_version;	
	var selectionEvent = new cpr.events.CUIEvent("execute-menu", {
		content: {
			"Target":DLG_ALMARAI_AUTHLOG_IMAGE,
			"InitVal": {
				"menu_id" : menu_id,
				"logImage": dmAuthLogImage.getValue("LogImage"),
				"logTerminalID": dmAuthLogImage.getValue("TerminalID"),
				"logTerminalName": dmAuthLogImage.getValue("TerminalName"),
				"logUserID": dmAuthLogImage.getValue("UserId"),
				"logUserName": dmAuthLogImage.getValue("UserName"),
				"logAuthResult": dmAuthLogImage.getValue("AuthResult"),
				"logEventTime": dmAuthLogImage.getValue("EventTime"),
				"logLogIndex": dmAuthLogImage.getValue("LogIndex"),
				"left": Almarai_AuthImageLeft,
				"top": Almarai_AuthImagetop,
				"width" : Almarai_AuthImageWidth,
				"height" : Almarai_AuthImageHeight
			}
		}
	});
	app.getHostAppInstance().dispatchEvent(selectionEvent); 
...
function onMRMAN_grdTerminalListContextmenu(/* cpr.events.CMouseEvent */ e){
	...
	// 단말기 출입문 원격 제어 설정
	var terminalRemoteDoor = terminalRow.getValue("RemoteDoor");
	
	var terminalMenu = new cpr.controls.Menu();
	var isListBox = false;
	if (ENABLE_MCP040 == 0) {
		if (terminalRemoteDoor == 1) {		// 모든 기능 사용 막음
			terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_OptionTerminalOptionSetting"), 32, ""));	
		} else if (terminalRemoteDoor == 2) {	// 출입문 개방만 허용
...
//			terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_TerminalLockControl"), 33, ""));
//			terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_TerminalUnLockControl"), 34, ""));
			terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_OptionTerminalOptionSetting"), 32, ""));
		} else {	// 모든 기능 사용 허용 	terminalRemoteDoor == 0
			...
			terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_TerminalUnLockControl"), 34, ""));
			terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_OptionTerminalOptionSetting"), 32, ""));	
			terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_Reboot"), 35, ""));
		}
	}
	else {
		var TerminalLiveInfo = app.lookup("TerminalLiveInfo");		
		...
		if(find_mcp == 1) {// MCP040 
			if (terminalRemoteDoor == 1) {	// 모든 기능 막음
				terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_OptionTerminalOptionSetting"), 32, ""));
			} else if (terminalRemoteDoor == 2) {	// 출입문 개방만 허용
...
//				terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_TerminalUnLockControl"), 34, ""));
				...
				terminalMenu = new cpr.controls.ListBox(); // 아이템이 너무 많아서 알페타에서 메뉴가 잘려서 보임....리스트 박스로 스크롤 적용
				terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_DoorCommandOpenTemp") + "1", 4, ""));
				terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_DoorCommandOpenTemp") + "2", 5, ""));
				terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_DoorCommandOpenTemp") + "3", 6, ""));
				terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_DoorCommandOpenTemp") + "4", 7, ""));
	...
	var topPx = (e.clientY - rect.top);
	var menucount = terminalMenu.getItemCount();
	var menuSize = 27;
	var mHeight = 100;
	
	if (isListBox){
		mHeight = 150;
		if (topPx + mHeight > rect.height) {
			topPx = topPx - mHeight;
		}		
	} else {
		if (topPx + menuSize * menucount > rect.height) {
			topPx = topPx - menuSize * menucount;
		}
	}
	...
	terminalMenu.addEventListener("selection-change", function(e) {
		switch (terminalMenu.value) {
			...
			*/
			...
			case "32": 	terminalOptionSetting();		break;
			
			case "33":	processTerminalLockCommand(1);	break;		// 단말기 폐쇄
			case "34":	processTerminalLockCommand(0);	break;		// 단말기 폐쇄 해제
			
			case "35":	processTerminalRebootCommand();	break;		// 단말기 재부팅
...
function onMRMAN_btnDoorControlClick(/* cpr.events.CMouseEvent */ e){
	...
	for( var i = 0; i < total; i++ ){
		...
		dsTerminalIDList.addRowData({"ID":row.getValue("ID"), "RemoteDoor":row.getValue("RemoteDoor")});
	}
	...
	}else if(cmbDoorControlOption.value == 33 || cmbDoorControlOption.value == 34){ // 단말기 폐쇄, 폐쇄 해제인 경우
		sendTerminalLockCommand();
	}else if(cmbDoorControlOption.value == 35){
		sendTerminalRebootCommand();
	}else{
...
function sendTerminalDoorCommand(){
	...
	if( row ){
		...
		if (ResultCode == COMERROR_NONE) {
			...
			dialogAlert(app, dataManager.getString("Str_Failed"), dataManager.getString("Str_AuditActionTransferFailed"));
			comLib.hideLoadMask();		
		}
	}
}
...
//단말기 옵션 설정 
function terminalOptionSetting(/* cpr.events.CSubmissionEvent */ e){
var path;
var grdTerminalList = app.lookup("MRMAN_grdTerminalList"); 
var terminalRow = grdTerminalList.getSelectedRow();		   
	if( TMMGR_brandType == BRAND_VRIDI) {		
		if(ENABLE_MCP040 == 1){
			var terminalType =  terminalRow.getValue("Type");		   
			if (terminalType == 21){ // MCP			
	...
	var selectionEvent = new cpr.events.CUIEvent("execute-menu", {
		content: {"Target":DLG_TERMINAL_INFO,	"InitVal": {"ID": terminalRow.getRowData()["ID"],"Path": path}}
	});
	app.getHostAppInstance().dispatchEvent(selectionEvent);
}
...
//이벤트기록 이벤트 그리드 - 우클릭(초기화)
function onMRMAN_grdEventlogContextmenu(/* cpr.events.CMouseEvent */ e){
	/** 
	 * @type cpr.controls.Grid
	 */
	var mRMAN_grdEventlog = e.control; //
	e.preventDefault();
	
	var terminalMenu = new cpr.controls.Menu();
	terminalMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_ListClear"), 0, ""));	

	var rect = app.getActualRect();
	var contextTop = (e.clientY - rect.top) > rect.height/2 ? e.clientY - rect.top*2 : e.clientY - rect.top;
	terminalMenu.style.css({
		left: e.clientX - rect.left + "px",
		top: (contextTop + rect.top ) + "px",
		height: "60px",
		width: "200px",
		position: "absolute"
	});
...
//이벤트 기록 리스트 초기화
function EventlogClear(/* cpr.events.CSubmissionEvent */ e){
	var count =	app.lookup("MRMAN_grdEventlog").getContentRowCount();
	for(var a=0; a<count; a++){
	app.lookup("MRMAN_grdEventlog").deleteRow(0);
	}	
}

// 단말기 잠금
function processTerminalLockCommand(lockCommand){
	var grdTerminalList = app.lookup("MRMAN_grdTerminalList");
	var terminalRow = grdTerminalList.getSelectedRow();
	if (terminalRow) {
		var terminalID = terminalRow.getValue("ID");
		var dmLockOption = app.lookup("LockOption");
		
		dmLockOption.setValue("TerminalID", terminalID);
		dmLockOption.setValue("Status", lockCommand);		// 0 : Unlock, 1 : Lock
		dmLockOption.setValue("Type", 1);					// 0 : Normal, 1 : Global
		
		var dmResult = app.lookup("Result");
		var sms_postTerminalLockControl = new cpr.protocols.Submission("sms_postTerminalLockdownControl");
		sms_postTerminalLockControl.action = "v1/terminals/" + terminalID + "/control/lockdown";
		sms_postTerminalLockControl.method = "post";
		sms_postTerminalLockControl.addRequestData(dmLockOption);				
		sms_postTerminalLockControl.addResponseData(dmResult);
		
		sms_postTerminalLockControl.addEventListenerOnce("submit-done", onSms_postTerminalLockControlExDone);
		sms_postTerminalLockControl.addEventListenerOnce("submit-error", onSms_postTerminalLockControlExError);
		sms_postTerminalLockControl.addEventListenerOnce("submit-timeout", onSms_postTerminalLockControlExTimeout);
		sms_postTerminalLockControl.send();
	}
}

function onSms_postTerminalLockControlExDone( /* cpr.events.CSubmissionEvent */ e) {
	comLib.updateLoadMask();
	sendTerminalLockCommand();
}

// 단말기 잠금 정보 수신 에러
function onSms_postTerminalLockControlExError( /* cpr.events.CSubmissionEvent */ e) {
	app.lookup("Result").setValue("ResultCode", COMERROR_NET_ERROR);
}

// 단말기 잠금 정보 수신 타임아웃
function onSms_postTerminalLockControlExTimeout( /* cpr.events.CSubmissionEvent */ e) {
	app.lookup("Result").setValue("ResultCode", COMERROR_NET_TIMEOUT);
}

function sendTerminalLockCommand() {
	var cmbDoorControlOption = app.lookup("MRMAN_cmbDoorControlOption");
	var lockCommand = cmbDoorControlOption.value;
	...
	if( row ){
		var terminalID = row.getValue("ID");
		console.log(row.getValue("RemoteDoor"));
		dsTerminalIDList.deleteRow(0);
		dsTerminalIDList.commit();
		var dmLockOption = app.lookup("LockOption");		
...
// 인증기록 리스트 우클릭 이벤트
function onMRMAN_grdAuthlogContextmenu(/* cpr.events.CMouseEvent */ e){
	/** 
	 * @type cpr.controls.Grid
	 */
	var mRMAN_grdAuthlog = e.control;
	e.preventDefault();
	
	var authlogMenu = new cpr.controls.Menu();
	authlogMenu.addItem(new cpr.controls.MenuItem(dataManager.getString("Str_ColumnSetting"), 0, ""));
	...
	authlogMenu.addEventListener("selection-change", function(e) {
		switch (authlogMenu.value) {
			case "0":	authlogListGridSetting();	break; 
			case "1":	AuthlogClear();         break;
			default:	break;
		}		
		authlogMenu.dispose();
	});	
	authlogMenu.addEventListener("blur", function(e) {	authlogMenu.dispose();	});
...
function authlogListGridSetting(){
	app.getRootAppInstance().openDialog("app/main/monitoring/MonitoringGrid", {
		width: 570,
		height: 570
		}, function(dialog){
...
function GridSetting(returnValue) {
	...
	var grdAuthlog = app.lookup("MRMAN_grdAuthlog");
	...
	if(oemVersion == OEM_ITONE_TRDATA || oemVersion == OEM_ITONE_POSCO_DX){
		// 명칭 변경  그룹 -> 협력사 
		// 사용자 ID 제외, 유니크 아이디 -> 사용자 아이디
		app.lookup("tabFolderID").getTabItemByID(1).bind("text").toLanguage("Str_PartnerCompany");
		grdAuthlog.deleteColumn(2);
//		app.lookup("MRMAN_grdAuthlog").header.getColumn(2).visible = false;
		grdAuthlog.header.getColumn(4).bind("text").toLanguage("Str_UserID");
	}
	
	grdAuthlog.resizableColumns = "all"; /* exbuilder 업데이트 시 적용됨 이번 릴리즈( 21/12/10 이후 ver) */
	initComboAuthLog();
	
}

//인증기록 리스트 초기화
function AuthlogClear(/* cpr.events.CSubmissionEvent */ e){
	var count =	app.lookup("MRMAN_grdAuthlog").getContentRowCount();
	for(var a=0; a<count; a++){
	app.lookup("MRMAN_grdAuthlog").deleteRow(0);
	}	
}


    
function grdTerminalReload(msg){
	var GrdTerminalList = app.lookup('GrdTerminalList');
	var TerminalList = app.lookup('TerminalList');
	
	for (var i = 0; msg.length > i; i++) {
		var terminalId = msg[i].ID; // 웹 소켓 terminal ID
		var grdTerminalRow = GrdTerminalList.findFirstRow("ID == " + terminalId);
		if (grdTerminalRow != null || grdTerminalRow != undefined) {
			var indexNum = grdTerminalRow.getValue('Rn');
			var findRowData = TerminalList.findFirstRow("ID == " + terminalId);
			
			GrdTerminalList.setValue(indexNum, 'Status', findRowData.getValue('Status'));
			GrdTerminalList.setValue(indexNum, 'TerminalStatus1', findRowData.getValue('TerminalStatus1'));
			GrdTerminalList.setValue(indexNum, 'TerminalStatus2', findRowData.getValue('TerminalStatus2'));
			GrdTerminalList.setValue(indexNum, 'TerminalStatus3', findRowData.getValue('TerminalStatus3'));
			GrdTerminalList.setValue(indexNum, 'TerminalStatus4', findRowData.getValue('TerminalStatus4'));
		}
	}
	GrdTerminalList.commit();
	app.lookup('MRMAN_grdTerminalList').redraw();
...
function sendTerminalRebootCommand(){
	...
	if (row) {
		...
		var dmRebootOption = app.lookup("RebootOption");		
		
		dmRebootOption.setValue("TerminalID", terminalID);
		
		var dmResult = app.lookup("Result");
		var sms_postTerminalRebootControl = new cpr.protocols.Submission("sms_postTerminalRebootControl");
		sms_postTerminalRebootControl.action = "v1/terminals/restart/" + terminalID;
		sms_postTerminalRebootControl.method = "post";
		sms_postTerminalRebootControl.addRequestData(dmRebootOption);				
		sms_postTerminalRebootControl.addResponseData(dmResult);
		
		sms_postTerminalRebootControl.addEventListenerOnce("submit-done", onSms_postTerminalRebootControlSubmitDone);
		sms_postTerminalRebootControl.addEventListenerOnce("submit-error", onSms_postTerminalRebootControlSubmitError);
		sms_postTerminalRebootControl.addEventListenerOnce("submit-timeout", onSms_postTerminalRebootControlSubmitTimeout);
		
		sms_postTerminalRebootControl.send();
	} else {
		dialogAlert(app, dataManager.getString("Str_Success"), dataManager.getString("Str_SendCommondComplete"));
		comLib.hideLoadMask();
	}
...
// 단말기 재부팅
function processTerminalRebootCommand(){
	var grdTerminalList = app.lookup("MRMAN_grdTerminalList");
	var terminalRow = grdTerminalList.getSelectedRow();
	if (terminalRow) {
		var terminalID = terminalRow.getValue("ID");
...
function onSms_postTerminalRebootControlSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	comLib.updateLoadMask();
	sendTerminalRebootCommand();
}
function onSms_postTerminalRebootControlSubmitError(/* cpr.events.CSubmissionEvent */ e){
	app.lookup("Result").setValue("ResultCode", COMERROR_NET_ERROR);	
}
function onSms_postTerminalRebootControlSubmitTimeout(/* cpr.events.CSubmissionEvent */ e){
	app.lookup("Result").setValue("ResultCode", COMERROR_NET_ERROR);
}
 ...
 */
function onMRMAN_grdAuthlogClick( /* cpr.events.CMouseEvent */ e) {
	/** 
	 * @type cpr.controls.Grid
	 */
	var mRMAN_grdAuthlog = e.control;
	if (app.lookup('MRMAN_grdAuthlog').getSelectedRow()) {
		
		/* '인증 로그'에서 선택한 row의 터미널ID 와 터미널 ID가 일치하는 '단말기 리스트' row 객채 */
		var selectTerminalRow = app.lookup('GrdTerminalList').findFirstRow('ID==' + app.lookup('MRMAN_grdAuthlog').getSelectedRow().getValue('TerminalID'));
		
		var grdTerminalList = app.lookup('MRMAN_grdTerminalList');
		
		/* 일치하는 row가 있다면(존재하는 터미널 id)해당 row로 단말기 list focus */
		if (selectTerminalRow) {
			grdTerminalList.focusCell(selectTerminalRow.getIndex());
		} else {
			grdTerminalList.clearSelection();
		}
	}
}
 ...
 * 사용자가 컨트롤을 클릭할 때 발생하는 이벤트.
 */
function onMRMAN_grdEventlogClick(/* cpr.events.CMouseEvent */ e){
	/** 
	 * @type cpr.controls.Grid
	 */
	var mRMAN_grdEventlog = e.control;
	if (app.lookup('MRMAN_grdEventlog').getSelectedRow()) {
		
		/* '이벤트 로그'에서 선택한 row의 터미널ID 와 터미널 ID가 일치하는 '단말기 리스트' row 객채 */
		var selectTerminalRow = app.lookup('GrdTerminalList').findFirstRow('ID==' + app.lookup('MRMAN_grdEventlog').getSelectedRow().getValue('TerminalID'));
...
/*
 * "Reset Emergency" 버튼(MRMAN_btnBoskFireAlarmClear)에서 click 이벤트 발생 시 호출.
 * 사용자가 컨트롤을 클릭할 때 발생하는 이벤트.
 */
function onMRMAN_btnBoskFireAlarmClearClick(/* cpr.events.CMouseEvent */ e){
	
	var grdTerminalList = app.lookup("MRMAN_grdTerminalList");
	var totalCnt = grdTerminalList.getRowCount() //연결된 단말기만
	...
	for( var i = 0; i < totalCnt; i++ ){
		var row = grdTerminalList.getRow(i);
		if( row.getValue("Status") == 0 || row.getValue("Status") == 16){continue;}
		dsTerminalIDList.addRowData({"ID":row.getValue("ID"), "RemoteDoor":row.getValue("RemoteDoor")});
	}
	dsTerminalIDList.commit();
	
	comLib.showLoadMask("pro", dataManager.getString("Str_Data")+" "+dataManager.getString("Str_Send"), "", totalCnt);
