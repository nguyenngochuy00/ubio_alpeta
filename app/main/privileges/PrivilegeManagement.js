...
function onBodyLoad( /* cpr.events.CEvent */ e) {
	...
	if(oemVersion == OEM_ITONE_TRDATA || oemVersion == OEM_ITONE_POSCO_DX){
		// 명칭 변경 및 사용않는 메뉴 삭제
		app.lookup("opbGroup").bind("value").toLanguage("Str_PartnerCompany");
		app.lookup("grp26").visible = false;
		app.lookup("grp18").visible = false;
		app.lookup("grp30").visible = false;
		app.lookup("grp31").visible = false;
	}
...
function changeMode(mode) {
	...
	if (PRMGR_mode == 0) { // 신규 입력
		...
		app.lookup("PRMGR_nbePrivilegeID").enabled = true;
		app.lookup("TMMGR_btnRegistPrivilege").enabled = true;
		app.lookup("TMMGR_btnUpdatePrivilege").enabled = false;
		app.lookup("TMMGR_btnDeletePrivilege").enabled = false;
		...
		// 아이디 수정 불가
		app.lookup("PRMGR_nbePrivilegeID").enabled = false;
		app.lookup("TMMGR_btnRegistPrivilege").enabled = true;
		app.lookup("TMMGR_btnUpdatePrivilege").enabled = true;
		app.lookup("TMMGR_btnDeletePrivilege").enabled = true;
	}
}
// 권한 리스트에서 권한 선택 시
function onTMMGR_grdPrivilegeListSelectionChange( /* cpr.events.CSelectionEvent */ e) {
	...
	if (row) {
		...
		requestData.action = "/v1/privileges/" + row.getValue("PrivilegeID");
...
function onTMMGR_btnRegistPrivilegeClick( /* cpr.events.CMouseEvent */ e) {
	if (PRMGR_mode == 0) { // 추가 모드인 경우
		...
		var IsExist = app.lookup("PrivilegeList").findFirstRow("PrivilegeID ==  " + privilegeID);
		if (IsExist) {
			dialogAlert(app, dataManager.getString("Str_Warning"), dataManager.getString("Str_ErrorPrivilegeDuplicateID"));
			return;
		}
...
function onSms_registPrivilegeInfoSubmitDone( /* cpr.events.CSubmissionEvent */ e) {
	...
	if (result == COMERROR_NONE) {
		...
		var newRow = privList.insertRowData(privList.getRowCount(), true, addBasic.getDatas());
		privList.commit();
		
		var grdPrivilegeList = app.lookup("PRMGR_grdPrivilegeList");
		grdPrivilegeList.select({
			rowIndex: newRow.getIndex()
		});
...
function onTMMGR_btnUpdatePrivilegeClick( /* cpr.events.CMouseEvent */ e) {
	if (PRMGR_mode == 0) {
		...
		smsUpdateprivilegeInfo.action = "/v1/privileges/" + privilegeID;
...
function onTMMGR_btnDeletePrivilegeClick( /* cpr.events.CMouseEvent */ e) {
	...
	if (privilegeRow) {
		if (dataManager.getOemVersion() == OEM_HYUNDAI_MSEAT) {
			var privilegeID = app.lookup("BasicInfo").getValue("PrivilegeID");
			if (privilegeID == 9001 || privilegeID == 9002 || privilegeID == 9003) {
				dialogAlert(app, dataManager.getString("Str_Warning"), "방문객 관리자 권한은 삭제할 수 없습니다.");
				return;
			}
		}
...
function onSms_updatePrivilegeInfoSubmitDone( /* cpr.events.CSubmissionEvent */ e) {
	...
	if (resultCode == 0) {
		dialogAlert(app, dataManager.getString("Str_Success"), dataManager.getString("Str_PrivilegeUpdateSuccess"));
	} else {
