	...
	cpr.core.App.load(url, function(loadedApp){
		...
		for (var i = 0; i < buttons.length; i++) {
			if (selectedButton == buttons[i]) {
				...
				buttons[i].style.css("backgroundColor", "#0083C6");
				buttons[i].style.css("border-bottom", "2px #FEC100 solid");		
			} else {
...
function applyOption() {
	...
	for (i = idxSubMission; i < count; i++) {				
		if (IS_INIT[i]) {
			...
			if (OPTIONS[i] =="NetworkInfo") {
				console.log(initInfo.getDatas());
				console.log(curInfo.getDatas());	
			}
			
			var resultflag = util.compareDataMap(initInfo, curInfo);
			console.log("resultflag" + resultflag);
			if (util.compareDataMap(initInfo, curInfo) == false) {
				console.log("compareDataMap");
				var requestData = app.lookup(SUB_MISSIONS[i]);
				requestData.action = "/v1/terminals/" + terminalID + ACTIONS[i];
				requestData.send();
				console.log(SUB_MISSIONS[i]);
				console.log(requestData.action);
				break;
			}
		}
...
function onSms_get_terminal_infoSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	...
	if (ResultCode == COMERROR_NONE) {
		...
		app.lookup("TMST2_Type").value = strType;
	} else {
		// error 
...
//----------------------------------------------------------------------------------> set_terminal_option_setting_config
...
function onTMST2_btnSaveClick(/* cpr.events.CMouseEvent */ e){
 ...
 */
function onTMST2_btnDeleteClick(/* cpr.events.CMouseEvent */ e){
