...
var ENABLE_MCP040 = 0;
var oem_version;
var modifiedList; // 수정된 탭 기록 (VictoryArch)
var productID;
var coreFalg;
...
function OptDataInit() {
	...
	app.lookup("TerminalCustomHDHI").clear();
	app.lookup("TMVBA_opbLock").value = "LOCK_TIMEZONE";
	if (oem_version == OEM_KANGWONLAND || oem_version == OEM_NEC) {
		app.lookup("TMVBA_opbLock").enabled = true;
		app.lookup("TMVBA_cmbLockTime").enabled = true;
		app.lookup("TMVBA_cmbLockTime").value = 0;
		
	} else {
		app.lookup("TMVBA_cmbLockTime").value = 0;
		app.lookup("TMVBA_opbLock").enabled = false;
		app.lookup("TMVBA_cmbLockTime").enabled = false;
		app.lookup("TMVBA_cmbLockTime").visible = false;
		app.lookup("TMVBA_opbLock").visible = false;
	}	
}
...
//		app.lookup("TMVBA_grbCore").getLayout().removeColumns([0,1]); 
		app.lookup("TMVBA_grbCore").getLayout().setColumns(["1px", "1px", "100px", "1fr", "140px", "80px", "80px", "14px"]); 
	}

//  Core 지원하는 경우만 보이도록 할 것이므로 get_terminal_info submitdone으로 이동
//	if (productID == ProductCore || productID == ProductCoreAndMobile){
//		app.lookup("TMVBA_cbxCore").visible = true;
//	}
	...
	} else if (oem_version == OEM_HYUNDAI_EC) {
		app.lookup("TMVBA_btnHIoSRegist").visible = true;// 버튼 활성화
	} else if (oem_version == OEM_3D_NORMAL ) {
	...
	if (ENABLE_MCP040 == 1) {
		var opMcpReaderIndex = app.lookup("opMcpReaderIndex");
		opMcpReaderIndex.visible = true;
		
		var opMcpTerminalID = app.lookup("opMcpTerminalID");
		opMcpTerminalID.visible = true;
		
		var cmbMcpReaderIndex = app.lookup("cmbMcpReaderIndex");
		cmbMcpReaderIndex.visible = true;
		cmbMcpReaderIndex.enabled = false;
		
		var cmbMcpTerminalID = app.lookup("cmbMcpTerminalID");
		cmbMcpTerminalID.visible = true;
		cmbMcpTerminalID.addItem(new cpr.controls.Item("-------", 0));
		cmbMcpTerminalID.value = 0;
		cmbMcpTerminalID.selectItem(0);
		cmbMcpTerminalID.redraw();
		
	}
	
	if (oem_version != OEM_HYUNDAI_HI) { // 현대 중공업 아니면 설치 위치 탭 삭제
		app.lookup("TMVBA_IN_grpButtons").getLayout().removeColumns([0]);
	} else {
//		app.lookup("TMVBA_btnLocation").click();
	}
	
	var outRegisterFlag = app.lookup("out_registerFlag");
	var cmbRegisterFlag = app.lookup("cmb_registerFlag");
	if (oem_version == OEM_BOSK_CAPS) {
		outRegisterFlag.visible = true;
		cmbRegisterFlag.visible = true;
	} else {
		outRegisterFlag.visible = false;
		cmbRegisterFlag.visible = false;
	}
	...
	if (oem_version == OEM_MOTORCYCLE_PARK) {
		app.lookup("TMVBA_opbDiscription").visible = false;
		app.lookup("TMVBA_Des_ipb").visible = false;
		
		app.lookup("TMVBA_cmbLockTime").enabled = true;
		app.lookup("TMVBA_cmbLockTime").visible = true;
		app.lookup("TMVBA_opbLock").enabled = true;
		app.lookup("TMVBA_opbLock").visible = true;
		
		app.lookup("TMVBA_cmbLockTime").unbind("value");
		app.lookup("TMVBA_opbLock").bind("value").toLanguage("Str_Setting");
		
		app.lookup("TMVBA_cmbLockTime").deleteAllItems();
		app.lookup("TMVBA_cmbLockTime").addItem(new cpr.controls.Item("-------", "0"));
		app.lookup("TMVBA_cmbLockTime").addItem(new cpr.controls.Item("IN", "1"));
		app.lookup("TMVBA_cmbLockTime").addItem(new cpr.controls.Item("OUT", "2"));	
		app.lookup("TMVBA_cmbLockTime").bind("value").toDataMap(app.lookup("TerminalInfo"), "Description");
		app.lookup("TMVBA_cmbLockTime").redraw();
		app.lookup("TMVBA_opbLock").redraw();
	}else if (oem_version == OEM_ARMY_HQ || oem_version == OEM_ITONE_TRDATA || oem_version == OEM_ITONE_POSCO_DX || dataManager.getOemVersion() == OEM_ROKMCH) {
	...
	} else if (oem_version == OEM_VICTORYARCH){
		// 어느 부분이 수정 되었는지 표기 용(VictoryArch)
		// [기본 옵션, 네트워크, 공휴일, 사이렌, 식수]
		modifiedList = [false,false,false,false,false];
		app.lookup("TMVBA_Des_ipb").inputFilter = "[^$/]"; // $는 수정 된 탭 전달시 구분자로 사용할 에정이라 입력 못하도록 설정
	} else if (oem_version == OEM_GS_BASIC){
		app.lookup('grpTerminalInfo').getLayout().setRowVisible(3, false);
		app.lookup('grpTerminalInfo').getLayout().setRowVisible(4, false);
		app.lookup('grpTerminalInfo').getLayout().setRowVisible(8, false);
		app.lookup('TMVBA_IN_grpButtons').getLayout().setColumnVisible(3, false);
		app.lookup('TMVBA_IN_grpButtons').getLayout().setColumnVisible(5, false);
	}
...
//	console.log(app.lookup("terminalOptflag").getDatas());
	return tOf;
}
...
exports.setHolidayOption = function(hOptData) {
	...
	hOptData[2].copyToDataMap(TerminalHolidayID)
	return result;
}
...
//<------------------------------------------------------------------------------------ Voip opt End
//----------------------------------------------------------------------------------> Terminal Location
exports.getTerminalLocationHDHI = function() {
	var TerminalLocationHDHI = app.lookup("TerminalCustomHDHI");
	return TerminalLocationHDHI;
}

exports.setTerminalLocationHDHI = function(nOptData) {
	var TerminalLocationHDHI = app.lookup("TerminalCustomHDHI");
	TerminalLocationHDHI.clear();
	var result = nOptData.copyToDataMap(TerminalLocationHDHI);
	return result;
}

exports.modifyTerminalLocationHDHI = function(modiMoptinfo) {
	var mOi = app.lookup("TerminalCustomHDHI");
	var beforeValue = mOi.getValue(modiMoptinfo[0]);
	if (beforeValue == modiMoptinfo[1]) {
		return -1;
	} else {
		mOi.setValue(modiMoptinfo[0], modiMoptinfo[1]);
		app.lookup("terminalOptflag").setValue("LocationOpt", 2);
	}
	return mOi;
}
...
function onSms_get_terminal_infoSubmitDone( /* cpr.events.CSubmissionEvent */ e) {
	...
	var cbxCore = app.lookup("TMVBA_cbxCore");
	if (ResultCode == 0) {
		...
		// Core 지원 단말만 보이도록 수정
			if(strType == "UBio-X Face Premium" || strType == "UBio-X Face Pro") {
				app.lookup("TMVBA_cbxCore").visible = true;
			}
		}
		...
		if (oem_version == OEM_REMOTE_FAW_MANAGEMENT){ 
			if (TerminalInfo.getValue("UseAuth") == UseAuthTerminal) { 
				setCustomRemoteFawTerminal();					
			}
		}
		
		//단말기 사진 이노뎁 버전용 (이미지 변경)
		if (oem_version == OEM_INNODEP_NORMAL) {
...
function onTMVBA_SaveClick( /* cpr.events.CMouseEvent */ e) {
	...
	if (app.lookup("TMVBA_cbxCore").checked == true){
...
function onSms_put_terminal_infoSubmitDone( /* cpr.events.CSubmissionEvent */ e) {
	...
	if (errorCode == COMERROR_NONE) {
		...
		if (oem_version == OEM_VICTORYARCH){
			// 어느 부분이 수정 되었는지 표기 용(VictoryArch)
			modifiedList = [false,false,false,false,false]; // 수정 성공하면 초기화
		}
...
function SendTransferItemstoTerminal() {
	...
	if (tOptflag.getValue("BasicOpt") == 2) {
		...
		console.log("send Submission :" + requestData.action);
		requestData.send();
	} else if (tOptflag.getValue("LocationOpt") == 2) {
		if (oem_version == OEM_HYUNDAI_HI){
			// 직영 관리자(Master)만  단말기 위치 수정 가능
			if (dataManager.getAccountInfo().getValue("UserID") == 1000000000000000000){
				requestData = app.lookup("sms_put_terminal_custom_HDHI");
				requestData.action = "/v1/hdhi/terminals/location/" + terminalID;
				console.log("send Submission :" + requestData.action);
				requestData.send();							
			} else {
				tOptflag.setValue("LocationOpt", 1);
			}
		} else {
			tOptflag.setValue("LocationOpt", 1);
		}
	...
	if (resultCode == COMERROR_NONE) {
		switch (sms_put_terminal_opt.id) {
			...
			case "sms_put_terminal_lock":
				...
				break;
			case "sms_put_terminal_custom_HDHI": // 현대중공업
				OptFlag.setValue("LocationOpt", 1);
				break;	
		...
		switch (sms_put_terminal_opt.id) {
			...
			case "sms_put_terminal_lock":
				strMsg = "lock";
				break;
			case "sms_put_terminal_custom_HDHI":
				strMsg = dataManager.getString("Str_InstallationLocation");
				break;	
		}
...
exports.setModifiedList = function(index){
	modifiedList[index] = true;
	//console.log(modifiedList);
}

function onCmb_registerFlagSelectionChange(/* cpr.events.CSelectionEvent */ e){
	var cmb_registerFlag = e.control;
	SvrSendFlag = 1;
}

function onTMVBA_cbxCoreValueChange(/* cpr.events.CValueChangeEvent */ e){
	SvrSendFlag = 1;
}

function setCustomRemoteFawTerminal(){
	app.lookup("TMVBA_opbUseAuth").visible = true;
	app.lookup("TMVBA_cmbfunctionType").enabled = false;
	app.lookup("cmb1").enabled = false;
	app.lookup("TMVBA_cmbSoftPassback").enabled = false;
	app.lookup("cmb_utcIndex").enabled = false;
	app.lookup("TMVBA_INN_embpage").enabled = false;
	app.lookup("TMVBA_IN_grpButtons").enabled = false;
}
