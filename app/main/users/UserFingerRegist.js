...
function onSms_getUserFPInfoSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	...
	if( dmResultCode == 0){	
		...
		//dialogAlert(app, "Warning", " 등록된 지문이 없습니다.");
	}else {	
...
function onUSFPR_btnApplyClick2(/* cpr.events.CMouseEvent */ e){
	// if UserFPInfo.length < 1 이면 삭제요청. 아니면 그냥 닫기
...
// 도움말 클릭
function onUSFPR_imgHelpPageClick(/* cpr.events.CMouseEvent */ e){
	var menu_id = 83886082;// 사용자 정보 페이지 열기	
...
function onUSFPR_udcTerminalListTerminalListClick(/* cpr.events.CSelectionEvent */ e){
	
	var SelectedTerminalInfo = app.lookup("USFPR_udcTerminalList");
	var checkedRowIndices = SelectedTerminalInfo.getCheckedRowIndices();
	if (checkedRowIndices.length >= 1 ) {
		dialogAlert(app, dataManager.getString("Str_Info"), dataManager.getString("Str_SelectedOneTerminal"));
		SelectedTerminalInfo.clearSelection();
		return;
	}
}