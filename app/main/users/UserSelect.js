...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	if (dataManager.getOemVersion() == OEM_ALMARAI_AUTHINFO) {
		udcUserSelectList.deleteColumn([14,13,12,11,10,9,7,6,5,4]);
	}else {
	...
	if (dataManager.getOemVersion() == OEM_ALMARAI_AUTHINFO) {
		udcUserTerminal.deleteUserColumn([14,13,12,11,10,9,7,6,5,4]);
	}else {
		udcUserTerminal.deleteUserColumn([14,13,12,11,10,9,8,7,6,5,4]);
	}
	udcUserTerminal.hideSubGroupCbx();
	...
	if(oemVersion == OEM_ITONE_TRDATA || oemVersion == OEM_ITONE_POSCO_DX){
		...
		var udcSelectUserGrid = app.lookup("USSEL_udcUserSelectList");
		udcSelectUserGrid.changeColumnNameGroupToPartner();
		udcSelectUserGrid.deleteColumn([1]);
	}	
}
...
function onUSSEL_btnApplyClick(/* cpr.events.CMouseEvent */ e){
	...
	var appName = app.getHost().userAttr("appName");
	//console.log(appName);
	if (appName == "mealPersonal"){ // 개인별 식수 조회는 사용자 한명만 선택 가능
		//console.log(selectIDMap.size);
		if(selectIDMap.size > 1){
			dialogAlert(app, dataManager.getString("Str_Warning"), dataManager.getString("Str_MealPersonalSelectOneError"));
			return;
		}
	}
