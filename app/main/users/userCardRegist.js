...
function onBodyLoad( /* cpr.events.CEvent */ e) {
	...
	USCDR_mode = initValue["Mode"];
	USCDR_url = initValue["Url"];
	
	if (USCDR_mode == "Regist") {
		...
		app.lookup("USCDR_btnCardDelete").visible = false;
		//app.lookup("USCDR_btnApply").visible = false;
	}
		...
		switch (msg.msgId) {
			case WSCmdCardCaptureRes: { // 캡쳐 완료. 결과 수신.
				...
				if (result.Result == "success") {
					...
					var oemVer = dataManager.getOemVersion();
					if (oemVer == OEM_DJMCITYHALL) {
						
						//strCardNum = "11111-222";
						
						var arrCardNum = strCardNum.split("-");
						if (arrCardNum.length == 2) {
							
							console.log("strCardNum1: " + strCardNum);
							
							if (arrCardNum[1].length == 3) {
								strCardNum = arrCardNum[0] + "00" + arrCardNum[1];
							} else if (arrCardNum[1].length == 4) {
								strCardNum = arrCardNum[0] + "0" + arrCardNum[1];
							} else {
								strCardNum = arrCardNum[0] + "" + arrCardNum[1];
							}
							
							console.log("strCardNum2: " + strCardNum);
						}
					}
...
function onSms_getUserCardInfoToTerminalSubmitDone( /* cpr.events.CSubmissionEvent */ e) {
	...
	var bResultCode = app.lookup("Result").getValue("ResultCode");
	if (USCDR_mode == "Scan") {
		if (bResultCode == ERROR_RFCARD_DUPLICATE) {
			bResultCode = COMERROR_NONE;
		}
	}
	if (bResultCode == COMERROR_NONE) {
