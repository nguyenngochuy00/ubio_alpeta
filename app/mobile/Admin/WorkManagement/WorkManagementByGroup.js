...
function onBlackListRowClickHandler(/* cpr.events.CEvent */ e) {
	...
	var periodTime = app.getAppProperty("periodTime");
	isAnimating = true;
	cpr.core.App.load("app/mobile/Admin/WorkManagement/WorkManagementGroupDetail", function(loadedApp){
	...
	app.openDialog("app/mobile/Admin/WorkManagement/WorkManagement", dialogProp, function(loadedApp) {
		...
		loadedApp.addEventListenerOnce("init", function () {
			loadedApp.setAppProperties({
				UserID: auth.getAuthenticatedUser().UserID
			});
		})
	});	
}
...
function onBodyInit(/* cpr.events.CEvent */ e){
	app.lookup("header").setAppProperties({
		pageName: cpr.I18N.INSTANCE.message("Str_Work_GroupMember")
	});
}
