...
function onVMVTR_btnFPRegistClick(/* cpr.events.CMouseEvent */ e){
	...
	app.getRootAppInstance().openDialog(appld, {width : 640, height : 490}, function(dialog){
		...
		if( count > 0 ){app.lookup("VMVTR_opbFPRegist").value="O";}
...
function onVMVTR_btnFAWRegistClick(/* cpr.events.CMouseEvent */ e){
	...
	app.getRootAppInstance().openDialog(appld, {width : 780, height : 500}, function(dialog){
		...
		var tmpInfo = returnValue.TemplateData;
		
		dsUserFaceWTInfo.addRow();
		dsUserFaceWTInfo.setValue(0,"TemplateSize",tmpInfo.TemplateSize);
		dsUserFaceWTInfo.setValue(0,"TemplateData",tmpInfo.TemplateData);
		dsUserFaceWTInfo.setValue(0,"TemplateType",tmpInfo.TemplateType);
...
function onVMVTR_btnVisitorAddClick(/* cpr.events.CMouseEvent */ e){
	...
	if (OEM_VERSION == OEM_3D_NORMAL) {
		...
		if (locale == "ko") {
			...
			userInfo.setValue("Name", visitorInfo.getValue("FirstName")+" "+visitorInfo.getValue("LastName"));
		}
	} else {
		userInfo.setValue("Name", visitorInfo.getValue("FirstName")+" "+visitorInfo.getValue("LastName"));
	}
