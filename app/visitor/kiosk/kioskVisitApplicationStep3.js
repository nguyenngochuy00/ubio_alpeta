...
/*
 * 아웃풋(VMVAS2_btnPrev)에서 click 이벤트 발생 시 호출.
 * 사용자가 컨트롤을 클릭할 때 발생하는 이벤트.
 */
function onVMVAS2_btnPrevClick(/* cpr.events.CMouseEvent */ e){
	/** 
	 * @type cpr.controls.Output
	 */
	var vMVAS2_btnPrev = e.control;
	cpr.core.App.load("app/visitor/kiosk/kioskVisitApplicationStep2", function(loadedApp){
		app.close();
		var instance = loadedApp.createNewInstance().run();
	});
}
...
function onVMVAS2_btnHomeClick(/* cpr.events.CMouseEvent */ e){
	...
	removeStepData();
	cpr.core.App.load("app/visitor/kiosk/kioskVisitorLogin", function(loadedApp){
		app.close();
		var instance = loadedApp.createNewInstance().run();
	});
}
