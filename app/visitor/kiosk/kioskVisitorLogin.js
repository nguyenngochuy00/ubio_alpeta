...
function onNextStep(step, data) {
	...
	if (step == 0) { // 방문 정보
		app.getRootAppInstance().openDialog("app/visitor/kiosk/visitApplicationStep1", {
			width: rect.width,
			height: rect.height
		}, function(dialog) {
			dialog.initValue = {
...
//	var oem_version = dataManager.getOemVersion();
	var appId = "app/visitor/kiosk/kioskVisitorPolicy";
 ...
 */
function onGroupFocusin(/* cpr.events.CFocusEvent */ e){
	/** 
	 * @type cpr.controls.Container
	 */
	var group = e.control;
	var ipb = app.lookup("VMLOG__ipbMobile");
	var output = app.lookup("VMLOG___opbMobile");
	ipb.style.css("color","#3b81c9");
	output.style.css("background-image", "url('../../../theme/images/kioskVisit/mobileIcon_1.png')");
}
 ...
 */
function onGroupFocusout(/* cpr.events.CFocusEvent */ e){
	/** 
	 * @type cpr.controls.Container
	 */
	var group = e.control;
	var ipb = app.lookup("VMLOG__ipbMobile");
	var output = app.lookup("VMLOG___opbMobile");
	ipb.style.css("color","#aeaeae");
	output.style.css("background-image", "url('../../../theme/images/kioskVisit/mobileIcon_1-1.png')");
}
 ...
 * 컨트롤 및 컨트롤의 하위 요소가 포커스를 획득하기 직전 발생하는 이벤트.
 */
function onPwGroupFocusin(/* cpr.events.CFocusEvent */ e){
	/** 
	 * @type cpr.controls.Container
	 */
	var pwGroup = e.control;
	var ipb = app.lookup("VMLOG_ipbPassword");
	var output = app.lookup("VMLOG___opbPw");
	ipb.style.css("color","#3b81c9");
	output.style.css("background-image", "url('../../../theme/images/kioskVisit/pwIcon_2-1.png')");
}
 ...
 * 컨트롤 밑 컨트롤의 하위 요소가 포커스를 잃기 직전 발생하는 이벤트.
 */
function onPwGroupFocusout(/* cpr.events.CFocusEvent */ e){
	/** 
	 * @type cpr.controls.Container
	 */
	var pwGroup = e.control;
	...
	var output = app.lookup("VMLOG___opbPw");
	ipb.style.css("color","#aeaeae");
	output.style.css("background-image", "url('../../../theme/images/kioskVisit/pwIcon_2.png')");
}
