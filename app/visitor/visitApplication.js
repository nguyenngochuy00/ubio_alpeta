...
var VMVAP_ver =0;
var VMVAP_mode = 0; //0:add,1:modify
...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	if( VMVAP_ver == 3 ){
		...
		app.lookup("VMVAP_opb1").value = "참가신청";
		app.lookup("VMVAP_opb1").redraw();
		
		app.lookup("VMVAP_opb2").unbind("value");
		app.lookup("VMVAP_opb2").value = "참가신청 후 처리현황을 조회하시려면 비밀번호를 반드시 입력하십시오";
		app.lookup("VMVAP_opb2").redraw();
		
		app.lookup("VMVAP_opb3").unbind("value");
		app.lookup("VMVAP_opb3").value = "참가정보";
		
		app.lookup("VMVAP_opb4").unbind("value");
		app.lookup("VMVAP_opb4").value = "행사명";
		
		app.lookup("VMVAP_opb5").unbind("value");
		app.lookup("VMVAP_opb5").value = "행사시작일시";
		
		app.lookup("VMVAP_opb6").unbind("value");
		app.lookup("VMVAP_opb6").value = "행사종료일시";
		
		app.lookup("VMVAP_opb7").unbind("value");
		app.lookup("VMVAP_opb7").value = "행사일정";
		
		app.lookup("VMVAP_opb8").unbind("value");
		app.lookup("VMVAP_opb8").value = "참가자 정보";
		
		app.lookup("VMVAP_opb9").unbind("value");
		app.lookup("VMVAP_opb9").value = "참가자 리스트";
		
		app.lookup("VMVAP_grpVisitInfo").redraw();
		
		app.lookup("VMVAP_btnVisitRequest").unbind("value");
		app.lookup("VMVAP_btnVisitRequest").value = "참가신청";
	}
}
...
exports.setModify = function(ver, mode, visitIndex){
	VMVAP_ver = ver	
	VMVAP_mode = mode	
	var visitInfo = app.lookup("VisitInfo");
	visitInfo.setValue("VisitIndex", visitIndex);
...
function onSms_getLangListSubmitDone(/* cpr.events.CSubmissionEvent */ e){	
	...
	sms_getLangList.send();	
	if(VMVAP_mode==1){ // 수정모드인 경우 버튼 문구 변경
		app.lookup("VMVAP_btnVisitRequest").bind("value").toLanguage("Str_Modification");
	}
}
...
function onSms_getLangErrorListSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	...
	if( VMVAP_mode == 1){
		...
		sms_getVisitApplication.action = "/v1/visitor/visitApplication/"+visitInfo.getValue("VisitIndex");;
	...
	app.openDialog("app/visit/userSearch", {width : width, height : height}, function(dialog){		
		...
		dialog.addEventListenerOnce("close", function(e){
			...
			if(result){
				...
				if( VMVAP_ver == 3 ){
					dmVisitInfo.setValue("Purpose", name);
				}
...
function onButtonClick(/* cpr.events.CMouseEvent */ e){
	if( VMVAP_ver == 3 ){
		cpr.core.App.load("app/custom/nhedu_03/visitorLogin", function(newapp) {
			app.close();
...
function onVMVAP_btnVisitRequestClick(/* cpr.events.CMouseEvent */ e){	
	...
	app.lookup("VMVAP_btnVisitRequest").enabled = false;
	if( VMVAP_mode == 0 ){
		var sms_postVisitApplication = app.lookup("sms_postVisitApplication");
		sms_postVisitApplication.send();
	}else {
		var sms_putVisitApplication = app.lookup("sms_putVisitApplication");
...
function onSms_postVisitApplicationSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	...
	if( dmResult.getValue("ResultCode") == COMERROR_NONE){
		app.lookup("VMVAP_btnVisitRequest").enabled = false; 
		if( VMVAP_ver == 3 ){			
			dialogAlert(app, dataManager.getString("Str_Success"), "참가신청이 완료되었습니다.", function(/*cpr.controls.Dialog*/dialog){			
				dialog.addEventListenerOnce("close", function(e) {
					var visitInfo = app.lookup("VisitInfo");
					var password = visitInfo.getValue("Password");
					if(password.length>0){
							
							var visitorList = app.lookup("VisitorList");
						var visitor = visitorList.getRow(0);					
						cpr.core.App.load("app/visitor/visitApplicationList", function(newapp) {
														
									
							app.close();		
							newapp.createNewInstance().run().callAppMethod("setVisitorInfo", 
								visitor.getValue("FirstName"),visitor.getValue("LastName"),
								visitor.getValue("Birthday"),password);								
						});
					}else {
						cpr.core.App.load("app/custom/nhedu_03/visitorLogin", function(newapp) {
							app.close();
		...
		app.lookup("VMVAP_btnVisitRequest").enabled = true;
...
function onSms_getVisitApplicationSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	...
	var sms_getVisitApplication = e.control;
	var resultCode = app.lookup("Result").getValue("ResultCode");
	if( resultCode == COMERROR_NONE){
		var visitInfo = app.lookup("VisitInfo");
		app.lookup("VMVAP_obpVisitTargetName").value = visitInfo.getValue("VisitTargetUserName");
		app.lookup("VMVAP_obpVisitTargetName").value = visitInfo.getValue("VisitTargetUserName");
		app.lookup("VMVAP_obpVisitTargetGroup").value = visitInfo.getValue("VisitTargetGroupName") + " / " + visitInfo.getValue("VisitTargetPositionName");
		var strStart = visitInfo.getValue("StartAt").split(' ');
		strStart[0] = strStart[0].replace(/-/gi,"")
		app.lookup("VMVAP_dtiStartDate").value = strStart[0];
		app.lookup("VMVAP_dtiStartTime").value = strStart[1];
		
		var strEnd = visitInfo.getValue("EndAt").split(' ');
		strEnd[0] = strEnd[0].replace(/-/gi,"")	
		app.lookup("VMVAP_dtiEndDate").value = strEnd[0];
		app.lookup("VMVAP_dtiEndTime").value = strEnd[1];
		app.lookup("VMVAP_grpVisitInfo").redraw();
	}else{
...
// 방문 신청 정보 수정 완료
function onSms_putVisitApplicationSubmitDone(/* cpr.events.CSubmissionEvent */ e){
	/** @type cpr.protocols.Submission	 */
	var sms_putVisitApplication = e.control;
	var resultCode = app.lookup("Result").getValue("ResultCode");
	if( resultCode == COMERROR_NONE){
		dialogAlert(app, dataManager.getString("Str_Success"), dataManager.getString("Str_VisitApplicationModified"));
	} else {
		dialogAlert(app, dataManager.getString("Str_Failed"), dataManager.getString(getErrorString(resultCode)));
	}
}
...
// 방문 신청 정보 수정 에러
function onSms_putVisitApplicationSubmitError(/* cpr.events.CSubmissionEvent */ e){
	app.lookup("Result").setValue("ResultCode",COMERROR_NET_ERROR)
}

// 방문 신청 정보 수정 타임아웃
function onSms_putVisitApplicationSubmitTimeout(/* cpr.events.CSubmissionEvent */ e){
	app.lookup("Result").setValue("ResultCode",COMERROR_NET_TIMEOUT)
}
...
function rotateImage( direction){
	...
	/*
		var count = Math.abs(rotateAngle / 90);
		console.log(rotateAngle,count);
		if( rotateAngle > 0 ) {
