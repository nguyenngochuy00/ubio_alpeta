...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	loginUserID = dataManager.getAccountID();
	if(isLoginUserPrivAdmin()){ // 출입구역 수정은 Master와 관리자만 가능하도록 변경
		app.lookup("UAAIC_btnAreaSave").enabled = true;
		app.lookup("UAAIC_btnAreaSave").visible = true;
		app.lookup("UAAIC_btnAreaDelete").enabled = true;
		app.lookup("UAAIC_btnAreaDelete").visible = true;
		app.lookup("UAAIC_nbeAreaID").readOnly = false;
		app.lookup("UAAIC_nbeAreaID").spinButton = true;
		app.lookup("UAAIC_ipbAreaName").readOnly = false;
		app.lookup("UAAIC_btnTerminalUnregist").enabled = true;
		app.lookup("UAAIC_btnTerminalRegist").enabled = true;
		app.lookup("UAAIC_btnTerminalUnregistAll").enabled = true;
		app.lookup("UAAIC_btnTerminalRegistAll").enabled = true;
		udcRegistTerminalList.deleteColumn([13,12,11,10,9,8,7,6,5,4,3]);
		udcUnregistTerminalList.deleteColumn([13,12,11,10,9,8,7,6,5,4,3]);
	} else {
		udcRegistTerminalList.deleteColumn([0,13,12,11,10,9,8,7,6,5,4,3]);
		udcUnregistTerminalList.deleteColumn([0,13,12,11,10,9,8,7,6,5,4,3]);		
	}
	udcUnregistTerminalList.setPaging(0, 500, 3);
	...
	if( dmResult.getValue("ResultCode") == 0){
		...
		for( var i=0; i < terminalCnt; i++ ){
			...
			if( row ){
				...
				selectIDMap.set(row.getValue("ID"),row.getValue("GroupCode"));
			}
		}	
...
function onUAAIC_btnAreaSaveClick(/* cpr.events.CMouseEvent */ e){
	...
	* */
	...
	if( areaName.length < 1 ){
		comLib.alert("warning", "출입구역 이름을 입력하세요");	
		return;
	}
	...
	dmAccessAreaAdd.setValue("Timezone2",app.lookup("UAAIC_cmbTimezone").value);
	dmAccessAreaAdd.setValue("Timezone3",app.lookup("UAAIC_cmbTimezone").value);
	dmAccessAreaAdd.setValue("Timezone4",app.lookup("UAAIC_cmbTimezone").value);
