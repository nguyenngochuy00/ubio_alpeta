...
function onBodyLoad( /* cpr.events.CEvent */ e) {
	...
	var defaultfontSize = Math.floor(appwidth / 130) + "px";
	var fontSize1 = Math.floor(appwidth / 110) + "px";
	var fontSize2 = Math.floor(appwidth / 90) + "px";
	...
	dmfontSize.setValue('font5', defaultfontSize); // 인증결과, 사용자 이름
...
//	app.lookup('ADB_OptTerminalName').value = terminalName;
	app.lookup("ADBIT_OPT_TerminalName").value = terminalName;
}
...
exports.setAuthInfo = function(authInfo) {
	...
	if (app.lookup('sms_getUserPicture').status != 'SENDING') {
		...
		var tempTime = authInfo.EventTime.toString().replaceAll('-','.');
		...
		dmAuthLogImage.setValue('TerminalName', app.lookup('ADBIT_OPT_TerminalName').value);
//		dmAuthLogImage.setValue('EventTime', authInfo.EventTime);
		dmAuthLogImage.setValue('EventTime', tempTime.substr(2));
		...
		dmAuthLogImage.setValue('GroupName', authInfo.GroupName);
		dmAuthLogImage.setValue('ReserveType', authInfo.ReserveType);
		
		// 체온 있으면 온도 표출
		if(authInfo.ReserveType == 1) {  //ReserveType = 1  온도	
			var temp = setTemperature(authInfo.ReserveData);
			dmAuthLogImage.setValue('AuthType', temp); // 바인딩된 값 안바꿔도 되도록 AuthType에 저장 
			
			// 인증 방식 -> 체온   아웃풋 변경
			app.lookup("ADB_AuthType").bind("value").toLanguage("Str_BodyTemperature");
			
		} else { // 체온 없으면  인증 방식 표출
			app.lookup("ADB_AuthType").bind("value").toLanguage("Str_AuthType");
			initComboAuthLog();			
		}
...
function initComboAuthLog() {
	...
	// 인증 이상 배경 css 
	if (app.lookup("AuthLogImage").getValue("AuthResult") != "0") {
		// 실패
		app.lookup('layOutGroup').style.css({
