...
exports.unVisibleColumn = function(indices){
	if (indices==undefined || indices == null ){
		return;
	}
	var gridUserList = app.lookup("UDC_grdUserList");
	...
	indices.forEach(function(index){
		console.log(index);
		gridUserList.columnVisible(index, false);
	});	
};
...
exports.findInnerUserList = function(category,keyword){
	...
	if (user) {
		...
		if (category == "Name") {
			dialogAlertAMHQ(app, "Waning", dataManager.getString("Str_ErrorNoNameFound"));
		} else if (category == "UniqueID") {
			dialogAlertAMHQ(app, "Waning", dataManager.getString("Str_ErrorNoUniqueIDFound"));
		}
 ...
 */
function onUDC_grdUserListCellClick(/* cpr.events.CGridMouseEvent */ e){
	if (oem_version == OEM_ARMY_HQ || dataManager.getOemVersion() == OEM_ROKMCH) {
		var clickRowIndex = e.rowIndex;
		
		if(e.cellIndex == 0){ // 체크컬럼 누를 경우
			return;
		}
		
		// 다중선택 가능하도록 변경
		var checkeIndices = app.lookup("UDC_grdUserList").getCheckRowIndices();
		if(app.lookup("UDC_grdUserList").isCheckedRow(clickRowIndex)){
			app.lookup("UDC_grdUserList").setCheckRowIndex(clickRowIndex, false);
		} else {
			app.lookup("UDC_grdUserList").setCheckRowIndex(clickRowIndex, true);
		}
