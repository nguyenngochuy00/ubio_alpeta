...
exports.setGroupList = function(/*cpr.data.DataSet*/groupList){
	...
	if( groupList && groupList.getRowCount()>0){
		...
		var cmbDept = app.lookup("TerminalList_cmbDept");
		...
		for ( var i = 0; i < count; i++ ){			
			...
			if (dataManager.getOemVersion() == OEM_HYUNDAI_HI){
				cmbDept.addItem(new cpr.controls.Item(groupInfo.getValue("Name"),groupInfo.getValue("GroupID")));
			}
		}
	...
	if (filter == "connect") { 
		...
		for (var i=0; i < rowCount; i++) {
			...
			if (dataManager.getOemVersion() == OEM_REMOTE_FAW_MANAGEMENT ) {
				if (rowData.getValue("UseAuth") != UseAuthTerminal){
					if(Loginuserid != 1000000000000000000 && LoginPrivilege != 1) {
						if(group == "") { // 그룹이 아닌 단말기가 로그 on/off 됬을때 그룹코드 값이 "" 처리되 보여지는 현상 수정  - otk
							continue;
						} else {
...
exports.setTerminalListAMHQ = function( /*cpr.data.DataSet*/terminalDataSet, filter ){
	...
	if (filter == "register") { 
		...
		for (var i=0; i < rowCount; i++) {
			var rowData = terminalDataSet.getRow(i);
			var RegisterFlag = rowData.getValue("RegisterFlag");
			var group = rowData.getValue("GroupCode")
			console.log("RegisterFlag : " + RegisterFlag);
			if(Loginuserid != 1000000000000000000 && LoginPrivilege != 1) {
				if(group == "") { // 그룹이 아닌 단말기가 로그 on/off 됬을때 그룹코드 값이 "" 처리되 보여지는 현상 수정  - otk
					continue;
				} else {
...
exports.setTerminalListITONE = function( /*cpr.data.DataSet*/terminalDataSet, filter ){
	...
	terminalListSet.clear();
	 ...
	 //등록된 모든 단말기 표시
	terminalDataSet.setSort("RegisterFlag desc"); // 등록기 우선 표시
	...
	terminalList.redraw();
	var isExistRegister = terminalDataSet.getRow(0).getValue("RegisterFlag");
	if(isExistRegister == 1) {
		// 등롤기가 존재하면 기본 체크
		terminalList.setCheckRowIndex(0, true);
	}
...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	if (oemVersion == OEM_INNODEP_NORMAL) {
		app.lookup("cmb1").getItemByValue(33).label = "IUA-7100S"
		app.lookup("cmb1").getItemByValue(34).label = "IUC-5110S"
		app.lookup("cmb1").getItemByValue(41).label = "IFFR-SC7000"
		app.lookup("cmb1").getItemByValue(45).label = "IFFR-XFS"	
	}
	
	var oemVersion = dataManager.getOemVersion();
	if(dataManager.getOemVersion() == OEM_ARMY_HQ || dataManager.getOemVersion() == OEM_ROKMCH) {
		app.lookup("terminalListGrid").setCellValue(0, 2, dataManager.getString("Str_TerminalLocation"));
	} else if(oemVersion == OEM_ITONE_TRDATA || oemVersion == OEM_ITONE_POSCO_DX){
//		app.lookup("terminalListGrid").header.getColumn(3).bind("text").toLanguage("Str_PartnerCompany");
	} else if (oemVersion == OEM_HYUNDAI_HI){
		var cmbPartner = app.lookup("TerminalList_cmbPartner");
		...
		for (var i = 3; i < 11; i++){
			terminalListGrid.columnVisible(i, true);			
		}
		terminalListGrid.columnVisible(11, false);
	}
}
