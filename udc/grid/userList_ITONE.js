...
function onBodyLoad(/* cpr.events.CEvent */ e){	
	...
	if (oem_version == OEM_ITONE_TRDATA || oem_version == OEM_ITONE_POSCO_DX) {
		...
		for (var i=0; i<14; i++) {
			if(i == 1 || i == 4 || i == 6 || i == 11){
//			if( i == 4 || i == 6 || i == 11){ // UserID 표출 (사용자 사진 일괄 등록 기능 이용을 위해)
				// 왜인지 모르겠지만 배열로 한방에 넣는게 안먹음
				grdUserList.deleteColumn(i);
			}
		}
		
		// 그리드 컬럼 이름 바꿔야함!
		grdUserList.header.getColumn(3).bind("text").toLanguage("Str_UserID");
		grdUserList.header.getColumn(5).bind("text").toLanguage("Str_PartnerCompany");
		grdUserList.header.getColumn(7).bind("text").toLanguage("Str_JobName");
		
		
		// 삭제한 컬럼 인덱스로 하면 에러뜸. 삭제안한 부분으로 이동시켜야함 
		grdUserList.moveColumn(3, 2);
...
exports.realDeleteRow = function(index) {
	var userList = app.lookup("UserList_ITONE");	
...
exports.setUserList = function( /*cpr.data.DataSet*/userDataSet ){
	...
	var userList = app.lookup("UserList_ITONE");
	var userListSrc = app.lookup("UserListSrc_ITONE");
...
exports.clearUserList = function(  ){
	...
	var userListSet = app.lookup("UserList_ITONE");
	userListSet.clear();
...
exports.updateUserInfo = function( userInfoData ){
	...
	var dsUserList = app.lookup("UserList_ITONE");
